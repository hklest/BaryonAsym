...
const int variableentries = xentries;
vector<double> bins = { 0.0001, 0.0002, 0.0004, 0.001, 0.01 };
//string names for file paths of 5 Trees we need to access
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// DATA
string filekaonTree = "/nfs/dust/h1/group/gtustin/h1oo/H1LambdaC/KaonTree/mergedKaonTree06.root";

string filelambdaTree = "/nfs/dust/h1/group/gtustin/h1oo/H1LambdaC/LambdaTree/mergedLambdaTree06.root";
//END DATA
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//MC
...
//output files
string outputfilename = "/nfs/dust/h1/group/klesthen/LambdaAnalysis/AnalysisX.root";
string dummyfile = "/nfs/dust/h1/group/klesthen/LambdaAnalysis/dummyfile.root";
			...
			if (pT>0.5 && pT<3.5) {
				if (abs(Eta)<1.3) {
					if (Ys > 0.1 && Ys < 0.6) {
						if (Q2s > 5 && Q2s < 100) {
							//if (abs(Angle) <= 10.0) {
								if (x > binsvector[j] && x < binsvector[j + 1]) {
						...
						if (Q2s > 5 && Q2s < 100) {
							if (H1GenBool == 1) {
								if (abs(VtxZgen)<30) {
									if(GenElecEn>11){
										if (Empz > 35 && Empz < 70) {
											tempcheck = Q2s / (Ys * 101568);
											if (tempcheck > binsvector[j] && tempcheck < binsvector[j + 1]) {
												GenCounts[j] = GenCounts[j] + 1 * weight1 * weight2;
											}
										}
								    }
...
void AnalysisX() {
	...
	for (int a = 0; a < xentries; a++) {
		string namestr1 = "Lambda Tot Data x Bin " + to_string(a + 1);
		string namestr2 = "Lambda Reg Data x Bin " + to_string(a + 1);
		string namestr3 = "Lambda Bar Data x Bin " + to_string(a + 1);
		string namestr4 = "Lambda Tot MC x Bin " + to_string(a + 1);
		string namestr5 = "Lambda Reg MC x Bin " + to_string(a + 1);
		string namestr6 = "Lambda Bar MC x Bin " + to_string(a + 1);
		string namestr7 = "Kaon Data x Bin " + to_string(a + 1);
		string namestr8 = "Kaon MC x Bin " + to_string(a + 1);
	...
	//Analyzing Tot Lambda Counts for x bins for MC and Data
	cout << "Now analyzing tree " << filelambdaTree.c_str() << endl;
	LambdaTotDataCount = AnalyzeTree(new TFile(filelambdaTree.c_str(), "read"), "Lambda Tot", variableentries, 0, LambdaTotData, bins, "Lambda Tot Data x Extraction ", c3, c1);
	cout << "Now analyzing tree " << filelambdaTreeMC.c_str() << endl;
	LambdaTotMCCount = AnalyzeTree(new TFile(filelambdaTreeMC.c_str(), "read"), "Lambda Tot", variableentries, 0, LambdaTotMC, bins, "Lambda Tot MC x Extraction ", c3, c1);
	cout << "Now analyzing tree " << filelambdaTree.c_str() << endl;
	LambdaRegDataCount = AnalyzeTree(new TFile(filelambdaTree.c_str(), "read"), "Lambda Reg", variableentries, 0, LambdaRegData, bins, "Lambda Reg Data x Extraction ", c3, c1);
	LambdaRegMCCount = AnalyzeTree(new TFile(filelambdaTreeMC.c_str(), "read"), "Lambda Reg", variableentries, 0, LambdaRegMC, bins, "Lambda Reg MC x Extraction ", c3, c1);

	LambdaBarDataCount = AnalyzeTree(new TFile(filelambdaTree.c_str(), "read"), "Lambda Bar", variableentries, 0, LambdaBarData, bins, "Lambda Bar Data x Extraction ", c3, c1);
	LambdaBarMCCount = AnalyzeTree(new TFile(filelambdaTreeMC.c_str(), "read"), "Lambda Bar", variableentries, 0, LambdaBarMC, bins, "Lambda Bar MC x Extraction ", c3, c1);

	KaonDataCount = AnalyzeTree(new TFile(filekaonTree.c_str(), "read"), "K0", variableentries, 1, KaonData, bins, "Kaon Tot Data x Extraction ", c3, c1);
	KaonMCCount = AnalyzeTree(new TFile(filekaonTreeMC.c_str(), "read"), "K0", variableentries, 1, KaonMC, bins, "Kaon Tot MC x Extraction ", c3, c1);
	...
	double xerr[variableentries] = { 0.00005, 0.0001, 0.0003, 0.0045};
	double xvals[variableentries] = { 0.00015, 0.0003, 0.0007, 0.0055};
	...
	double dyup[4];
	int n = 0;
	X[n] = xvals[n];	y[n] = 20.1;	dxlow[n] = 0;	dxup[n] = 0;	dylow[n] = 0.35;	dyup[n] = 0.35;	n++;
	X[n] = xvals[n];	y[n] = 8.5;	dxlow[n] = 0;	dxup[n] = 0;	dylow[n] = 0.15;	dyup[n] = 0.15;	n++;
	X[n] = xvals[n];	y[n] = 2.57;	dxlow[n] = 0;	dxup[n] = 0;	dylow[n] = 0.016;	dyup[n] = 0.016;	n++;
	X[n] = xvals[n];	y[n] = 0.104;	dxlow[n] = 0;	dxup[n] = 0;	dylow[n] = 0.0015;	dyup[n] = 0.0015;	n++;
	//
	...
	for (int k = 0; k < variableentries; k++) {
		...
		cout << "Lambda Rad Cor x Bin " << k + 1 << ": " << templambdaradcorr << endl;

		double templamcrosssection = (LambdaTotDataCount[k][0]) / (1000*templameff * BRlambda * DataIntLumi06 * templambdaradcorr * (bins[k+1]-bins[k]));
	...
	double dyup2[4];
	int m = 0;
	dylow2[m] = 1.4;	dyup2[m] = 1.4;	m++;
	dylow2[m] = 0.5;	dyup2[m] = 0.5;	m++;
	dylow2[m] = 0.15;	dyup2[m] = 0.15;	m++;
	dylow2[m] = 0.006;	dyup2[m] = 0.006;	m++;
	
	//Plotting Lambda Efficiency
	TCanvas* ceff = new TCanvas("Lambda Cross Section vs x", "Lambda Cross Section vs x", 10, 10, 800, 400);
	ceff->SetName("Lambda Reconstruction Eff vs x");
	...
	effreg->SetTitle("#Lambda Reconstruction Eff vs x");
	effreg->GetXaxis()->SetTitle("x");
	...
	PlotData("Lambda Cross Section vs x", "d#sigma(ep #rightarrow e [#Lambda + #bar{#Lambda}] X)/dx", "x", "d#sigma/dx",0,4.2, cs, variableentries, X, y, xerr, y1err, dylow, dyup, dylow2, dyup2);
	for (int k = 0; k < variableentries; k++) {
		cout << "Lambda x Cross Section Entry " << k + 1 << " : " << y1[k] << " X: " << X[k] << "p/m: " << y1err[k] << endl;
	}
	...
	X[n] = xvals[n];	y[n] = 0.0001515;	dxlow[n] = 0;	dxup[n] = 0;	dylow[n] = 0.03287;	dyup[n] = 0.03287;	n++;
	X[n] = xvals[n];	y[n] = 0.0003015;	dxlow[n] = 0;	dxup[n] = 0;	dylow[n] = 0.03105;	dyup[n] = 0.03105;	n++;
	X[n] = xvals[n];	y[n] = -0.0007;	dxlow[n] = 0;	dxup[n] = 0;	dylow[n] = 0.0347;	dyup[n] = 0.0347;	n++;
	X[n] = xvals[n];	y[n] = -0.005459;	dxlow[n] = 0;	dxup[n] = 0;	dylow[n] = 0.02922;	dyup[n] = 0.02922;	n++;
	///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	PlotData("Lambda Asymm vs x", "[#sigma(ep #rightarrow e #Lambda X) - #sigma(ep #rightarrow e #bar{#Lambda} X)] / [#sigma(ep #rightarrow e #Lambda X) + #sigma(ep #rightarrow e #bar{#Lambda} X)]", "x", "A_{#Lambda}",-0.3,0.3, cs2, variableentries, X, y, xerr, y1err, dylow, dyup, dylow, dyup);
	
	//K0 Cross section results from 2009 H1 Paper
	n = 0;
	X[n] = xvals[n];	y[n] = 51.7;	dxlow[n] = 0;	dxup[n] = 0;	dylow[n] = 0.3;	dyup[n] = 0.3;	n++;
	X[n] = xvals[n];	y[n] = 24.0;	dxlow[n] = 0;	dxup[n] = 0;	dylow[n] = 0.15;	dyup[n] = 0.15;	n++;
	X[n] = xvals[n];	y[n] = 7.07;	dxlow[n] = 0;	dxup[n] = 0;	dylow[n] = 0.035;	dyup[n] = 0.035;	n++;
	X[n] = xvals[n];	y[n] = 0.315;	dxlow[n] = 0;	dxup[n] = 0;	dylow[n] = 0.002;	dyup[n] = 0.002;	n++;
	
	m = 0;
	dylow2[m] = 3.3;	dyup2[m] = 3.2;	m++;
	dylow2[m] = 1.5;	dyup2[m] = 1.4;	m++;
	dylow2[m] = 0.43;	dyup2[m] = 0.43;	m++;
	dylow2[m] = 0.019;	dyup2[m] = 0.019;	m++;
	...
	for (int k = 0; k < variableentries; k++) {
		...
		cout << "K0 Rad Cor x Bin " << k + 1 << ": " << tempk0radcorr << endl;

		double tempk0crosssection = (KaonDataCount[k][0]) / (1000*tempk0eff * BRkaon * DataIntLumi06 * tempk0radcorr * (bins[k + 1] - bins[k]));
	...
	TCanvas* ckeff = new TCanvas("K0 Cross Section vs x", "K0 Cross Section vs x", 10, 10, 800, 400);
	ckeff->SetName("K0 Reconstruction Eff vs x");
	...
	effreg2->SetTitle("K^{0}_{S} Reconstruction Eff vs x");
	effreg2->GetXaxis()->SetTitle("x");
	...
	cradcorr->SetName("Radiaitve Corrections vs x");
	...
	radcorrlambda->SetTitle("Radiative Corrections vs x");
	radcorrlambda->GetXaxis()->SetTitle("x");
	...
	PlotData("K0 Cross Section vs x", "d#sigma(ep #rightarrow e K^{0}_{S} X)/dx", "x", "d#sigma/dx [ub]",0,10.5, cs3, variableentries, X, y, xerr, y1err, dylow, dyup, dylow2, dyup2);
	for (int k = 0; k < variableentries; k++) {
		cout << "K0 x Cross Section Entry " << k + 1 << " : " << y1[k] << " X: " << X[k] << "p/m: " << y1err[k] << endl;
	}
	...
	n = 0;
	X[n] = xvals[n];	y[n] = 0.390;	dxlow[n] = 0;	dxup[n] = 0;	dylow[n] = 0.007;	dyup[n] = 0.007;	n++;
	X[n] = xvals[n];	y[n] = 0.355;	dxlow[n] = 0;	dxup[n] = 0;	dylow[n] = 0.0055;	dyup[n] = 0.0055;	n++;
	X[n] = xvals[n];	y[n] = 0.364;	dxlow[n] = 0;	dxup[n] = 0;	dylow[n] = 0.006;	dyup[n] = 0.006;	n++;
	X[n] = xvals[n];	y[n] = 0.329;	dxlow[n] = 0;	dxup[n] = 0;	dylow[n] = 0.0055;	dyup[n] = 0.0055;	n++;
	
	m = 0;
	dylow2[m] = 0.020;	dyup2[m] = 0.019;	m++;
	dylow2[m] = 0.013;	dyup2[m] = 0.012;	m++;
	...
	PlotData("Lambda/K0 Cross Section vs x", "[d#sigma(ep #rightarrow e #Lambda X)]/[d#sigma(ep #rightarrow e K^{0}_{S} X)]", "x", "#Lambda/K^{0}_{S}",0,0.51, cs4, variableentries, X, y, xerr, y1err, dylow, dyup, dylow2, dyup2);
	...
	for (int a = 0; a < 1; a++) {
		...
		string namestr2 = "Lambda Reg Data x Bin";
		string namestr3 = "Lambda Bar Data x Bin";
		string namestr4 = "Lambda Tot MC x Bin";
		string namestr5 = "Lambda Reg MC x Bin";
		string namestr6 = "Lambda Bar MC x Bin";
		string namestr7 = "Kaon Data x Bin";
		string namestr8 = "Kaon MC x Bin";
	...
	LambdaTotDataCountincl = AnalyzeTree(new TFile(filelambdaTree.c_str(), "read"), "Lambda Tot", 1, 0, LambdaTotDataxincl, { bins[0],bins[variableentries] }, "Lambda Tot Incl Data x Extraction ", c3, c1);
	LambdaTotMCCountincl = AnalyzeTree(new TFile(filelambdaTreeMC.c_str(), "read"), "Lambda Tot", 1, 0, LambdaTotMCxincl, { bins[0],bins[variableentries] }, "Lambda Tot Incl MC x Extraction ", c3, c1);

	LambdaRegDataCountincl = AnalyzeTree(new TFile(filelambdaTree.c_str(), "read"), "Lambda Reg", 1, 0, LambdaRegDataxincl, { bins[0],bins[variableentries] }, "Lambda Reg Incl Data x Extraction ", c3, c1);
	LambdaRegMCCountincl = AnalyzeTree(new TFile(filelambdaTreeMC.c_str(), "read"), "Lambda Reg", 1, 0, LambdaRegMCxincl, { bins[0],bins[variableentries] }, "Lambda Reg Incl MC x Extraction ", c3, c1);

	LambdaBarDataCountincl = AnalyzeTree(new TFile(filelambdaTree.c_str(), "read"), "Lambda Bar", 1, 0, LambdaBarDataxincl, { bins[0],bins[variableentries] }, "Lambda Bar Incl Data x Extraction ", c3, c1);
	LambdaBarMCCountincl = AnalyzeTree(new TFile(filelambdaTreeMC.c_str(), "read"), "Lambda Bar", 1, 0, LambdaBarMCxincl, { bins[0],bins[variableentries] }, "Lambda Bar Incl MC x Extraction ", c3, c1);

	KaonDataCountincl = AnalyzeTree(new TFile(filekaonTree.c_str(), "read"), "K0", 1, 1, KaonDataxincl, { bins[0],bins[variableentries] }, "Kaon Incl Data x Extraction ", c3, c1);
	KaonMCCountincl = AnalyzeTree(new TFile(filekaonTreeMC.c_str(), "read"), "K0", 1, 1, KaonMCxincl, { bins[0],bins[variableentries] }, "Kaon Incl MC x Extraction ", c3, c1);

	
	//LambdaTotDataCountincl = AnalyzeTree(new TFile(filelambdaTree.c_str(), "read"), "Lambda Tot", 1, 0, LambdaTotDataxincl, { 10,bins[variableentries] }, "Lambda Tot Incl Data x Extraction ", c3, c1);
	//LambdaTotMCCountincl = AnalyzeTree(new TFile(filelambdaTreeMC.c_str(), "read"), "Lambda Tot", 1, 0, LambdaTotMCxincl, { 10,bins[variableentries] }, "Lambda Tot Incl MC x Extraction ", c3, c1);

	//LambdaRegDataCountincl = AnalyzeTree(new TFile(filelambdaTree.c_str(), "read"), "Lambda Reg", 1, 0, LambdaRegDataxincl, { 10,bins[variableentries] }, "Lambda Reg Incl Data x Extraction ", c3, c1);
	//LambdaRegMCCountincl = AnalyzeTree(new TFile(filelambdaTreeMC.c_str(), "read"), "Lambda Reg", 1, 0, LambdaRegMCxincl, { 10,bins[variableentries] }, "Lambda Reg Incl MC x Extraction ", c3, c1);

	//LambdaBarDataCountincl = AnalyzeTree(new TFile(filelambdaTree.c_str(), "read"), "Lambda Bar", 1, 0, LambdaBarDataxincl, { 10,bins[variableentries] }, "Lambda Bar Incl Data x Extraction ", c3, c1);
	//LambdaBarMCCountincl = AnalyzeTree(new TFile(filelambdaTreeMC.c_str(), "read"), "Lambda Bar", 1, 0, LambdaBarMCxincl, { 10,bins[variableentries] }, "Lambda Bar Incl MC x Extraction ", c3, c1);

	//KaonDataCountincl = AnalyzeTree(new TFile(filekaonTree.c_str(), "read"), "K0", 1, 1, KaonDataxincl, { 10,bins[variableentries] }, "Kaon Incl Data x Extraction ", c3, c1);
	//KaonMCCountincl = AnalyzeTree(new TFile(filekaonTreeMC.c_str(), "read"), "K0", 1, 1, KaonMCxincl, { 10,bins[variableentries] }, "Kaon Incl MC x Extraction ", c3, c1);
