...
int Plot(TFile* output,double lowerbinrange, double upperbinrange, double bins, int variable, string canvasname, string plottitle, string xtitle, string yaxis) {
	...
	PYTHIAHist->GetYaxis()->SetTitleOffset(1.75);
	...
	canvas->cd();
	TPad* padR = new TPad("padR", "padR", 0, 0.3, 1, 1.0);
	...
	*PYTHIARatioHist = (*PYTHIAsave) / (*DataHist);
	...
	DataRatioHist->GetYaxis()->SetTitleOffset(1.85);
...
void DISControlPlotsPhotoproduction() {
	...
	cout << "Q2 done" << endl;
	Plot(outputfile, 0.0375, 0.6, 100, 1, "y Distributions", "y Distributions", "y", "1/Luminosity dN/dy [nb]");
	cout << "Y done" << endl;
