...
//Function for Guassian Kernel Smoothing
//double ySys1Smoothed[q2entries] = {};
vector<double> GuassianKernalSmoothing(double binwidtharray[q2entries], double UnsmoothedSys[q2entries], double xvalues[q2entries]) {

	//static double output[q2entries];
	vector<double> output = {};

	for (int p = 0; p < q2entries; p++) {
		double SmoothingSum = 0;
		double SmoothingSumWeight = 0;
		cout << "Calculating Smoothed Bin " << p + 1 << endl;
		cout << "Bin Value: " << xvalues[p] << endl;
		for (int b = 0; b < q2entries; b++) {
			double x_i = xvalues[p]; //used as the point we are evaluating at
			double x_j = xvalues[b]; //used as the x coordinate of the bin in secondary loop we are considering
			double tempweight = TMath::Gaus(x_j, x_i, 2 * binwidtharray[b], kTRUE);
			cout << "Weight from Bin " << b + 1 << ": " << tempweight << endl;
			cout << "Bin Width: " << binwidtharray[b] << endl;
			SmoothingSum = SmoothingSum + UnsmoothedSys[b] * tempweight;
			SmoothingSumWeight = SmoothingSumWeight + tempweight;

		}
		double tempSmoothedValue = SmoothingSum / SmoothingSumWeight;
		if (tempSmoothedValue < 0.85 * UnsmoothedSys[p]) {
			//outputarray[p] = 0.85 * UnsmoothedSys[p];
			output.push_back(0.85 * UnsmoothedSys[p]);
		}
		else {
			//outputarray[p] = tempSmoothedValue;
			output.push_back(tempSmoothedValue);
		}
		cout << "Original Value: " << UnsmoothedSys[p] << endl;
		cout << "Smoothed Value: " << output[p] << endl;
	}
...
vector<vector<double>> AnalyzeTreeOLD(int distvariableindex, TFile* Data, string TreeName, int binentries, int particlecode, vector<TH1F*> HistArray, vector<double> binsvector, string extractionReadback, TCanvas* c3, TCanvas* c1, vector<vector<double>> binparaminits, vector<vector<double>> binrangedefs) { //when calling function, particle code deictates 0=lambda or 1=kaon MLP parameters
	...
	for (int j = 0; j < binentries; j++) {
		...
		c3->SetName(extractionnamestr.c_str());
		c1->SetName(trainingnamestr.c_str());
		ExtractionDataVector.push_back(mlpfunction(HistArray[j], extractionReadback.c_str() + to_string(j + 1), particlecode, c3, binparaminits[j], binrangedefs[j]));
	}
	return ExtractionDataVector;
...
//This takes the raw gen MC trees and gets MC total counts for efficiency calculation in the appropraiate bin defined by inputs, function output is the count
vector<vector<double>> AnalyzeGenMatchedMCTree(int distvariableindex, TFile* Data, string TreeName, int binentries, vector<double> binsvector) { //when calling function, particle code deictates 0=lambda or 1=kaon MLP parameters

	vector<vector<double>> Counts = {};
	
	vector<double> StabilityNum = {};
	vector<double> StabilityDenom = {};
	vector<double> PurityNum = {};
	vector<double> PurityDenom = {};

	int p;
	for (p = 0; p < binentries; p++) {
		StabilityNum.push_back(0);
		StabilityDenom.push_back(0);
		PurityNum.push_back(0);
		PurityDenom.push_back(0);
	...
	//Defining the variables that are stored in the Tree branches
	//double invMasslambda, invMasskaon, pT, Eta, x, Q2s, Ys;
	double pT, Eta, x, Q2s, Ys, pTgen, Etagen, Q2sgen, Ysgen, xgen, weight1, weight2, invMasslambda;
	...
	vector<double> distvariable = { 0,0,0,0,0,0,0 };
	vector<double> distvariableGen = { 0,0,0,0,0,0,0 };
	int m;
	int j;
	for (m = 0; m < entries; m++) {
		tree->GetEntry(m);
				...
				if (abs(Eta) < 1.3) {
					if (Ys > lowerYsbound && Ys < upperYsbound) {
						if (Q2s > 5 && Q2s < 100) {
							...
							//distvariable = { Q2s, Q2s / (Ys * 101568), pT, Eta, genPhi, sqrt((DecayXVtx) * (DecayXVtx)+(DecayYVtx) * (DecayYVtx)), DecayZVtx };
							//distvariableGen = { Q2sgen, Q2sgen / (Ysgen * 101568), pTgen, Etagen, genPhi, sqrt((DecayXVtx) * (DecayXVtx)+(DecayYVtx) * (DecayYVtx)), DecayZVtx };
							distvariable = { Q2s, Q2s / (Ys * 101568), pT, Eta, 0, 0, 0 };
							distvariableGen = { Q2sgen, Q2sgen / (Ysgen * 101568), pTgen, Etagen, 0, 0, 0 };
							if (invMasslambda >1.104 && invMasslambda <1.126) {
								if ((distvariable[distvariableindex] > binsvector[j] && distvariable[distvariableindex] < binsvector[j + 1]) && (distvariableGen[distvariableindex] > binsvector[j] && distvariableGen[distvariableindex] < binsvector[j + 1])) {
									//If reconstructed AND generated in this bin, then count
									StabilityNum[j] = StabilityNum[j] + 1 * weight1 * weight2;
									PurityNum[j] = PurityNum[j] + 1 * weight1 * weight2;
								}
								if ((distvariableGen[distvariableindex] > binsvector[j] && distvariableGen[distvariableindex] < binsvector[j + 1])) {
									//If GENERATED in this bin, then count
									StabilityDenom[j] = StabilityDenom[j] + 1 * weight1 * weight2;

								}
								if ((distvariable[distvariableindex] > binsvector[j] && distvariable[distvariableindex] < binsvector[j + 1])) {
									//If RECONSTRUCTED AND generated in this bin, then count
									PurityDenom[j] = PurityDenom[j] + 1 * weight1 * weight2;
	...
	int r;
	for (r = 0; r < binentries; r++) {

		Counts[r][0] = StabilityNum[r] / StabilityDenom[r];
		Counts[r][1] = PurityNum[r] / PurityDenom[r];
		cout << "STABILITY BIN " << r + 1 << " " << Counts[r][0] << endl;
		cout << "PURITY BIN " << r + 1 << " " << Counts[r][1] << endl;
	}

	
	//Counts = { {Bin 1 Stability, Bin 1 Purity} , ...}
	return Counts;
...
void Asym(int varidx, vector<double> q2bins, double DataIntLumi06, double MCDjangoh14radgen, double MCDjangoh14nonradgen, string filekaonTree, string filelambdaTree, string filekaonTreeMC, string filelambdaTreeMCRAPGAP, string filelambdaTreeMCDJANGOH, string filegenMCTreeRAPGAP, string filegenMCTreeDJANGOH, string fileNonRadgenMCTree, string fileRadgenMCTree, string xaxistitle, vector<vector<double>> binparaminitsLambda, vector<vector<double>> binrangedefsLambda, vector<vector<double>> binparaminitsLambdaBar, vector<vector<double>> binrangedefsLambdaBar, vector<vector<double>> binparaminitsMCLambda, vector<vector<double>> binrangedefsMCLambda, vector<vector<double>> binparaminitsMCLambdaBar, vector<vector<double>> binrangedefsMCLambdaBar) {
	...
	c3->SetLeftMargin(0.15); //root default is 0.1
	...
	LambdaRegGenCountRAPGAP = AnalyzeGenTree(varidx, new TFile(filegenMCTreeRAPGAP.c_str(), "read"), "Lambda Reg", q2entries, q2bins);
	LambdaBarGenCountRAPGAP = AnalyzeGenTree(varidx, new TFile(filegenMCTreeRAPGAP.c_str(), "read"), "Lambda Bar", q2entries, q2bins);
	
	LambdaRegGenCountDJANGOH = AnalyzeGenTree(varidx, new TFile(filegenMCTreeDJANGOH.c_str(), "read"), "Lambda Reg", q2entries, q2bins);
	LambdaBarGenCountDJANGOH = AnalyzeGenTree(varidx, new TFile(filegenMCTreeDJANGOH.c_str(), "read"), "Lambda Bar", q2entries, q2bins);
	...
	//Stability, Purity, Resolution Plots
	//vector<vector<double>> AnalyzeGenMatchedMCTree(int distvariableindex, TFile* Data, string TreeName, int binentries, vector<double> binsvector);
	//LambdaBarBGGenCountRAPGAP = AnalyzeGenTree(varidx, new TFile(filegenMCTreeRAPGAP.c_str(), "read"), "Lambda Bar BG", q2entries, q2bins);
	vector<vector<double>> LambdaRegPRSCounts = AnalyzeGenMatchedMCTree(varidx, new TFile(filelambdaTreeMCRAPGAP.c_str(), "read"), "Lambda Reg Gen", q2entries, q2bins);
	vector<vector<double>> LambdaBarPRSCounts = AnalyzeGenMatchedMCTree(varidx, new TFile(filelambdaTreeMCRAPGAP.c_str(), "read"), "Lambda Bar Gen", q2entries, q2bins);
	double LamRegStability[q2entries] = {};
	double LamBarStability[q2entries] = {};
	double LamRegPurity[q2entries] = {};
	double LamBarPurity[q2entries] = {};
	...
	double sigregplotData[q2entries] = {};
	double sigbarplotData[q2entries] = {};
	double sigregplotRAPGAP[q2entries] = {};
	double sigbarplotRAPGAP[q2entries] = {};
	double sigregplotDJANGOH[q2entries] = {};
	double sigbarplotDJANGOH[q2entries] = {};
	double errsigregplotData[q2entries] = {};
	double errsigbarplotData[q2entries] = {};
	double errsigregplotRAPGAP[q2entries] = {};
	double errsigbarplotRAPGAP[q2entries] = {};
	double errsigregplotDJANGOH[q2entries] = {};
	double errsigbarplotDJANGOH[q2entries] = {};
	...
	double runningintegratedAsym = 0;
	double runningintegratedAsymerror = 0;
	///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	for (int k = 0; k < q2entries; k++) {
		...
		sigregplotData[k] = LambdaRegDataCount[k][10];
		sigbarplotData[k] = LambdaBarDataCount[k][10];
		sigregplotRAPGAP[k] = LambdaRegMCRAPGAPCount[k][10];
		sigbarplotRAPGAP[k] = LambdaBarMCRAPGAPCount[k][10];
		sigregplotDJANGOH[k] = LambdaRegMCDJANGOHCount[k][10];
		sigbarplotDJANGOH[k] = LambdaBarMCDJANGOHCount[k][10];
		errsigregplotData[k] = LambdaRegDataCount[k][11];
		errsigbarplotData[k] = LambdaBarDataCount[k][11];
		errsigregplotRAPGAP[k] = LambdaRegMCRAPGAPCount[k][11];
		errsigbarplotRAPGAP[k] = LambdaBarMCRAPGAPCount[k][11];
		errsigregplotDJANGOH[k] = LambdaRegMCDJANGOHCount[k][11];
		errsigbarplotDJANGOH[k] = LambdaBarMCDJANGOHCount[k][11];
		errsigbarplotDJANGOH[k] = LambdaBarMCDJANGOHCount[k][11];
		...
		double MCerrDirectDiff = abs(y1[k] - y2[k]);
		ySys1[k] = MCerrDirectDiff;
		...
		double templamregeffRAPGAPExtendedRange = (LambdaRegMCRAPGAPCount[k][4]) / (LambdaRegGenCountRAPGAP[k]);
		double templambareffRAPGAPExtendedRange = (LambdaBarMCRAPGAPCount[k][4]) / (LambdaBarGenCountRAPGAP[k]);
		double templamregRAPGAPExtendedRange = (LambdaRegDataCount[k][4]) / (templamregeffRAPGAPExtendedRange);
		double templambarRAPGAPExtendedRange = (LambdaBarDataCount[k][4]) / (templambareffRAPGAPExtendedRange);
		double ASYMRAPGAPExtendedRange = (templamregRAPGAPExtendedRange - templambarRAPGAPExtendedRange) / ((templamregRAPGAPExtendedRange + templambarRAPGAPExtendedRange));
		double extractionrange = abs(ASYMRAPGAPExtendedRange - y1[k]);
		...
		double templamregeffRAPGAPRawExtraction = (LambdaRegMCRAPGAPCount[k][5]) / (LambdaRegGenCountRAPGAP[k]);
		double templambareffRAPGAPRawExtraction = (LambdaBarMCRAPGAPCount[k][5]) / (LambdaBarGenCountRAPGAP[k]);
		double templamregRAPGAPRawExtraction = (LambdaRegDataCount[k][5]) / (templamregeffRAPGAPRawExtraction);
		double templambarRAPGAPRawExtraction = (LambdaBarDataCount[k][5]) / (templambareffRAPGAPRawExtraction);
		double ASYMRAPGAPRawExtraction = (templamregRAPGAPRawExtraction - templambarRAPGAPRawExtraction) / ((templamregRAPGAPRawExtraction + templambarRAPGAPRawExtraction));
		double rawextraction = abs(ASYMRAPGAPRawExtraction - y1[k]);
		...
		double templamregeffRAPGAPaltBGfit = (LambdaRegMCRAPGAPCount[k][6]) / (LambdaRegGenCountRAPGAP[k]);
		double templambareffRAPGAPaltBGfit = (LambdaBarMCRAPGAPCount[k][6]) / (LambdaBarGenCountRAPGAP[k]);
		double templamregRAPGAPaltBGfit = (LambdaRegDataCount[k][6]) / (templamregeffRAPGAPaltBGfit);
		double templambarRAPGAPaltBGfit = (LambdaBarDataCount[k][6]) / (templambareffRAPGAPaltBGfit);
		double ASYMRAPGAPaltBGfit = (templamregRAPGAPaltBGfit - templambarRAPGAPaltBGfit) / ((templamregRAPGAPaltBGfit + templambarRAPGAPaltBGfit));
		double alternativeBGfit = abs(ASYMRAPGAPaltBGfit - y1[k]);
		...
		double templamregeffRAPGAPNuclearInt = (LambdaRegMCRAPGAPCount[k][0]) / (LambdaRegGenCountRAPGAP[k]);
		double templambareffRAPGAPNuclearInt = (LambdaBarMCRAPGAPCount[k][0]) / (LambdaBarGenCountRAPGAP[k]);
		double templamregRAPGAPNuclearInt = (LambdaRegDataCount[k][0] + LambdaRegDataCount[k][0] * (averagepercentBGcontam)) / (templamregeffRAPGAPNuclearInt);
		double templambarRAPGAPNuclearInt = (LambdaBarDataCount[k][0]) / (templambareffRAPGAPNuclearInt);
		double ASYMRAPGAPNuclearInt = (templamregRAPGAPNuclearInt - templambarRAPGAPNuclearInt) / ((templamregRAPGAPNuclearInt + templambarRAPGAPNuclearInt));
		double detectorinterror = abs(ASYMRAPGAPNuclearInt - y1[k]);
		...
		//double MCerrDirectDiff = abs(y1[k]-y2[k]) / 2.0;
		yAsymmerrSYS[k] = sqrt(Square(MCerrDirectDiff) + Square(extractionrange) + Square(rawextraction) + Square(alternativeBGfit) + Square(detectorinterror));
		//yAsymmerrSYS[k] = 0.01;
		yAsymmTOTerr[k] = sqrt(Square(yAsymmerrSYS[k]) + Square(yAsymmerr[k]));

		//Statistical Error, using propogated RooFit error which includes Sqrt(N) err and fit parameter uncertainties propgated through the yield
		yAsymmerr[k] = MCSignalRooFitYieldError;
		...
		runningintegratedAsym = runningintegratedAsym + yAsymm[k]*2*xerr[k]/(q2bins[q2entries] - q2bins[0]);
		runningintegratedAsymerror = runningintegratedAsymerror + Square(yAsymmTOTerr[k]*(2 * xerr[k])/(q2bins[q2entries]-q2bins[0]));

		cout << "Asym in Bin " << k + 1 << " : " << yAsymm[k] << " +/- " << yAsymmerr[k] << " (stat.) " << " +/-" << yAsymmTOTerr[k] << " (sys.)" << endl;
		cout << "Bin " << k + 1 << " : " << xvals[k] << " +/- " << xerr[k] << endl;
}
	//Now that everything is calculated, I will get the Smoothed systematic uncertainty values and recalculate the total systematic uncertainty contributions in each bin
	double yAsymmerrSYSsmoothed[q2entries];
	vector<double> ySys1Smoothedvec = GuassianKernalSmoothing(binwidths, ySys1, xvals);
	vector<double> ySys3Smoothedvec = GuassianKernalSmoothing(binwidths, ySys3, xvals);
	vector<double> ySys4Smoothedvec = GuassianKernalSmoothing(binwidths, ySys4, xvals);
	vector<double> ySys5Smoothedvec = GuassianKernalSmoothing(binwidths, ySys5, xvals);
	vector<double> ySys6Smoothedvec = GuassianKernalSmoothing(binwidths, ySys6, xvals);
	double ySys1Smoothed[q2entries] = {};
	double ySys3Smoothed[q2entries] = {};
	double ySys4Smoothed[q2entries] = {};
	double ySys5Smoothed[q2entries] = {};
	double ySys6Smoothed[q2entries] = {};
	for (int i = 0; i < q2entries; i++) {
		//yAsymmerrSYS[k] = sqrt(Square(MCerrDirectDiff) + Square(extractionrange) + Square(rawextraction) + Square(alternativeBGfit) + Square(detectorinterror));
		//ySys 1,3,4,5,6
		ySys1Smoothed[i] = ySys1Smoothedvec[i];
		ySys3Smoothed[i] = ySys3Smoothedvec[i];
		ySys4Smoothed[i] = ySys4Smoothedvec[i];
		ySys5Smoothed[i] = ySys5Smoothedvec[i];
		ySys6Smoothed[i] = ySys6Smoothedvec[i];
		yAsymmerrSYSsmoothed[i] = sqrt(Square(ySys1Smoothed[i]) + Square(ySys3Smoothed[i]) + Square(ySys4Smoothed[i]) + Square(ySys5Smoothed[i]) + Square(ySys6Smoothed[i]));
	
	}

	//ZeroHist
	TH1F* ZeroHist = new TH1F("z", "z", 1000, -1000, 1000);
	ZeroHist->SetLineColor(kBlack);
	...
	LambdaBarRAPGAPeff->SetLineColor(kRed);
	LambdaRegDJANGOHeff->SetLineColor(kBlack);
	LambdaBarDJANGOHeff->SetLineColor(kRed);

	LambdaRegRAPGAPeff->SetMarkerColor(kBlack);
	LambdaBarRAPGAPeff->SetMarkerColor(kRed);
	LambdaRegDJANGOHeff->SetMarkerColor(kBlack);
	LambdaBarDJANGOHeff->SetMarkerColor(kRed);

	LambdaRegRAPGAPeff->SetMarkerStyle(21);
	LambdaBarRAPGAPeff->SetMarkerStyle(21);
	LambdaRegDJANGOHeff->SetMarkerStyle(22);
	LambdaBarDJANGOHeff->SetMarkerStyle(22);

	LambdaRegRAPGAPeff->SetMarkerSize(1.0);
	LambdaBarRAPGAPeff->SetMarkerSize(1.0);
	LambdaRegDJANGOHeff->SetMarkerSize(1.0);
	LambdaBarDJANGOHeff->SetMarkerSize(1.0);
	...
	TLegend* legend = new TLegend(0.5513, 0.4144, 0.888, 0.8823);
	legend->AddEntry(LambdaRegRAPGAPeff, "RAPGAP #epsilon_{#Lambda}", "p");
	legend->AddEntry(LambdaBarRAPGAPeff, "RAPGAP #epsilon_{#bar{#Lambda}}", "p");
	legend->AddEntry(LambdaRegDJANGOHeff, "DJANGOH #epsilon_{#Lambda}", "p");
	legend->AddEntry(LambdaBarDJANGOHeff, "DJANGOH #epsilon_{#bar{#Lambda}}", "p");
	...
	mgeff->GetYaxis()->SetTitleSize(0.06);
	mgeff->GetXaxis()->SetTitleSize(0.06);
	outputfile.cd();
	c3->SetGrid();
	c3->Write();
	f->cd();
	...
	//Plotting Data, RAPGAP, and DJANGOH Detector Resolutions (Sigma)
	TMultiGraph* mgsig = new TMultiGraph();
	c3->Clear();
	legend->Clear();
	c3->SetName("Detector Resolutions");

	TGraphAsymmErrors* LambdaRegDatasig = new TGraphAsymmErrors(q2entries, xvals, sigregplotData, xerr, xerr, errsigregplotData, errsigregplotData);
	TGraphAsymmErrors* LambdaBarDatasig = new TGraphAsymmErrors(q2entries, xvals, sigbarplotData, xerr, xerr, errsigbarplotData, errsigbarplotData);
	TGraphAsymmErrors* LambdaRegRAPGAPsig = new TGraphAsymmErrors(q2entries, xvals, sigregplotRAPGAP, xerr, xerr, errsigregplotRAPGAP, errsigregplotRAPGAP);
	TGraphAsymmErrors* LambdaBarRAPGAPsig = new TGraphAsymmErrors(q2entries, xvals, sigbarplotRAPGAP, xerr, xerr, errsigbarplotRAPGAP, errsigbarplotRAPGAP);
	TGraphAsymmErrors* LambdaRegDJANGOHsig = new TGraphAsymmErrors(q2entries, xvals, sigregplotDJANGOH, xerr, xerr, errsigregplotDJANGOH, errsigregplotDJANGOH);
	TGraphAsymmErrors* LambdaBarDJANGOHsig = new TGraphAsymmErrors(q2entries, xvals, sigbarplotDJANGOH, xerr, xerr, errsigbarplotDJANGOH, errsigbarplotDJANGOH);
	
	LambdaRegDatasig->SetLineColor(kBlack);
	LambdaBarDatasig->SetLineColor(kRed);
	LambdaRegRAPGAPsig->SetLineColor(kBlack);
	LambdaBarRAPGAPsig->SetLineColor(kRed);
	LambdaRegDJANGOHsig->SetLineColor(kBlack);
	LambdaBarDJANGOHsig->SetLineColor(kRed);

	LambdaRegDatasig->SetMarkerColor(kBlack);
	LambdaBarDatasig->SetMarkerColor(kRed);
	LambdaRegRAPGAPsig->SetMarkerColor(kBlack);
	LambdaBarRAPGAPsig->SetMarkerColor(kRed);
	LambdaRegDJANGOHsig->SetMarkerColor(kBlack);
	LambdaBarDJANGOHsig->SetMarkerColor(kRed);

	LambdaRegDatasig->SetMarkerStyle(21);
	LambdaBarDatasig->SetMarkerStyle(21);
	LambdaRegRAPGAPsig->SetMarkerStyle(22);
	LambdaBarRAPGAPsig->SetMarkerStyle(22);
	LambdaRegDJANGOHsig->SetMarkerStyle(20);
	LambdaBarDJANGOHsig->SetMarkerStyle(20);

	LambdaRegDatasig->SetMarkerSize(1.0);
	LambdaBarDatasig->SetMarkerSize(1.0);
	LambdaRegRAPGAPsig->SetMarkerSize(1.0);
	LambdaBarRAPGAPsig->SetMarkerSize(1.0);
	LambdaRegDJANGOHsig->SetMarkerSize(1.0);
	LambdaBarDJANGOHsig->SetMarkerSize(1.0);

	mgsig->Add(LambdaRegDatasig, "AP");
	mgsig->Add(LambdaBarDatasig, "AP");
	mgsig->Add(LambdaRegRAPGAPsig, "AP");
	mgsig->Add(LambdaBarRAPGAPsig, "AP");
	mgsig->Add(LambdaRegDJANGOHsig, "AP");
	mgsig->Add(LambdaBarDJANGOHsig, "AP");


	legend->AddEntry(LambdaRegDatasig, "Data #sigma_{#Lambda}", "p");
	legend->AddEntry(LambdaBarDatasig, "Data #sigma_{#bar{#Lambda}}", "p");
	legend->AddEntry(LambdaRegRAPGAPsig, "RAPGAP #sigma_{#Lambda}", "p");
	legend->AddEntry(LambdaBarRAPGAPsig, "RAPGAP #sigma_{#bar{#Lambda}}", "p");
	legend->AddEntry(LambdaRegDJANGOHsig, "DJANGOH #sigma_{#Lambda}", "p");
	legend->AddEntry(LambdaBarDJANGOHsig, "DJANGOH #sigma_{#bar{#Lambda}}", "p");
	//LambdaAsymRAPGAP->Draw("ap");
	//LambdaAsymDJANGOH->SetOption("same");
	//LambdaAsymDJANGOH->Draw();
	mgsig->Draw("AP");
	legend->Draw("same");
	mgsig->SetTitle("Detector Resolutions");
	mgsig->GetXaxis()->SetTitle(xaxistitle.c_str());
	mgsig->GetYaxis()->SetTitle("#sigma");
	//mgsig->GetYaxis()->SetTitleSize(0.05);
	//mgsig->GetXaxis()->SetTitleSize(0.05);
	outputfile.cd();
	c3->SetGrid();
	c3->Write();
	f->cd();
	...
	//Plotting Asymetries Measured in RAPGAP vs DJANGOH
	...
	//Plotting Main Results with unsmoothed Systematic Errors
	...
	LambdaAsym->SetLineWidth(2);
	LambdaAsymSYS->SetLineWidth(2);
	//TGraphAsymmErrors* LambdaAsymTOT = new TGraphAsymmErrors(q2entries, xvals, yAsymm, xerr, xerr, yAsymmTOTerr, yAsymmTOTerr);
	LambdaAsym->SetLineColor(kRed);
	LambdaAsymSYS->SetLineColor(kBlack);
	...
	mgAsym->GetYaxis()->SetTitleSize(0.06);
	mgAsym->GetXaxis()->SetTitleSize(0.06);
	outputfile.cd();
	c3->SetGrid();
	c3->Write();
	f->cd();

	//Plotting Main Results with SMOOTHED Systematic Errors
	TMultiGraph* mgAsymSmoothed = new TMultiGraph();
	c3->Clear();
	legend->Clear();
	c3->SetName("Asymmetry with Smoothed Systematic Uncertainties");
	//TGraphAsymmErrors* LambdaAsym = new TGraphAsymmErrors(q2entries, xvals, yAsymm, xerr, xerr, yAsymmerr, yAsymmerr);
	TGraphAsymmErrors* LambdaAsymSYSsmoothed = new TGraphAsymmErrors(q2entries, xvals, yAsymm, xerr, xerr, yAsymmerrSYSsmoothed, yAsymmerrSYSsmoothed);
	//TGraphAsymmErrors* LambdaAsymTOT = new TGraphAsymmErrors(q2entries, xvals, yAsymm, xerr, xerr, yAsymmTOTerr, yAsymmTOTerr);
	//LambdaAsym->SetLineColor(kRed);
	LambdaAsymSYSsmoothed->SetLineColor(kBlack);
	//LambdaAsymTOT->SetLineColor(kBlack);
	legend->AddEntry(LambdaAsym, "Statistical", "l");
	legend->AddEntry(LambdaAsymSYSsmoothed, "Systematic", "l");
	//LambdaAsymSmoothed->SetLineWidth(2);
	LambdaAsymSYSsmoothed->SetLineWidth(2);
	//legend->AddEntry(LambdaAsymTOT, "Total", "l");
	mgAsymSmoothed->Add(LambdaAsym, "AP");
	mgAsymSmoothed->Add(LambdaAsymSYSsmoothed, "AP");
	//mgAsym->Add(LambdaAsymTOT, "AP");
	mgAsymSmoothed->Draw("AP");
	ZeroHist->Draw("same");
	legend->Draw("same");
	mgAsymSmoothed->SetTitle("#frac{#sigma(ep #rightarrow e #Lambda X) - #sigma(ep #rightarrow e #bar{ #Lambda } X)}{#sigma(ep #rightarrow e #Lambda X) + #sigma(ep #rightarrow e #bar{ #Lambda } X)} [2005-2007 HERA II DST7]");
	mgAsymSmoothed->GetXaxis()->SetTitle(xaxistitle.c_str());
	mgAsymSmoothed->GetYaxis()->SetTitle("A_{#Lambda}");
	mgAsymSmoothed->GetYaxis()->SetTitleSize(0.06);
	mgAsymSmoothed->GetXaxis()->SetTitleSize(0.06);
	outputfile.cd();
	c3->SetGrid();
	c3->Write();
	f->cd();
	...
	mgRooErrCheck->GetYaxis()->SetTitleSize(0.06);
	mgRooErrCheck->GetXaxis()->SetTitleSize(0.06);
	outputfile.cd();
	c3->Update();
	c3->SetGrid();
	c3->Write();
	...
	//Plotting Individual Systematic Error Contributions (Not Smoothed)
	...
	STATerr->SetLineColor(kRed);
	legend->AddEntry(Sys1, "DJANGOH vs. RAPGAP", "l");
	//legend->AddEntry(Sys2, "RooFit Param Error + Statistical", "l");
	...
	mgSys->GetYaxis()->SetTitleSize(0.06);
	mgSys->GetXaxis()->SetTitleSize(0.06);
	outputfile.cd();
	c3->Update();
	c3->SetGrid();
	c3->Write();
	...
	//	double SmoothingSumWeight = 0;
	//	cout << "Calculating Smoothed Bin " << p + 1 << endl;
	//	cout << "Bin Value: " << xvals[p] << endl;
	//	for (int b = 0; b < q2entries; b++) {
	//		double x_i = xvals[p]; //used as the point we are evaluating at
	//		double x_j = xvals[b]; //used as the x coordinate of the bin in secondary loop we are considering
	//		double tempweight = TMath::Gaus(x_j, x_i, 2*binwidths[b], kTRUE);
	//		cout << "Weight from Bin " << b + 1 << ": " << tempweight << endl;
	//		cout << "Bin Width: " << binwidths[b] << endl;
	//		SmoothingSum = SmoothingSum + ySys1[b] * tempweight;
	//		SmoothingSumWeight = SmoothingSumWeight + tempweight;

	//	}
	//	ySys1Smoothed[p] = SmoothingSum / SmoothingSumWeight;
	//	cout << "Original Value: " << ySys1[p] << endl;
	//	cout << "Smoothed Value: " << ySys1Smoothed[p] << endl;
	//}
	//Plotting Individual Systematic Error Contributions (Smoothed)
	TMultiGraph* mgSysSmoothed = new TMultiGraph();
	c3->Clear();
	legend->Clear();
	c3->SetName("Smoothed Error Contributions");
	//double* GuassianKernalSmoothing(double binwidtharray[q2entries], double UnsmoothedSys[q2entries], double xvalues[q2entries]) {
	TGraphAsymmErrors* SmoothedSys1 = new TGraphAsymmErrors(q2entries, xvals, ySys1Smoothed, xerr, xerr, 0, 0);
	TGraphAsymmErrors* SmoothedSys2 = new TGraphAsymmErrors(q2entries, xvals, ySys2, xerr, xerr, 0, 0); //This is the same as STATerr, is counting statistics of Lambdas and the roofit parameter statistical uncertainties combined
	TGraphAsymmErrors* SmoothedSys3 = new TGraphAsymmErrors(q2entries, xvals, ySys3Smoothed, xerr, xerr, 0, 0);
	TGraphAsymmErrors* SmoothedSys4 = new TGraphAsymmErrors(q2entries, xvals, ySys4Smoothed, xerr, xerr, 0, 0);
	TGraphAsymmErrors* SmoothedSys5 = new TGraphAsymmErrors(q2entries, xvals, ySys5Smoothed, xerr, xerr, 0, 0);
	TGraphAsymmErrors* SmoothedSys6 = new TGraphAsymmErrors(q2entries, xvals, ySys6Smoothed, xerr, xerr, 0, 0);
	TGraphAsymmErrors* SmoothedSTATerr = new TGraphAsymmErrors(q2entries, xvals, yAsymmerr, xerr, xerr, 0, 0);
	SmoothedSys1->SetLineColor(kBlack);
	SmoothedSys2->SetLineColor(kRed);
	SmoothedSys3->SetLineColor(kBlue);
	SmoothedSys4->SetLineColor(kGreen);
	SmoothedSys5->SetLineColor(kMagenta);
	SmoothedSys6->SetLineColor(kOrange);
	SmoothedSTATerr->SetLineColor(kRed);
	legend->AddEntry(SmoothedSys1, "DJANGOH vs. RAPGAP", "l");
	//legend->AddEntry(Sys2, "RooFit Param Error + Statistical", "l");
	legend->AddEntry(SmoothedSys3, "Extraction Range", "l");
	legend->AddEntry(SmoothedSys4, "Raw Signal Extraction", "l");
	legend->AddEntry(SmoothedSys5, "Alternative BG Fit", "l");
	legend->AddEntry(SmoothedSys6, "Detector Interactions", "l");
	legend->AddEntry(SmoothedSTATerr, "Statistical", "l");
	mgSysSmoothed->Add(SmoothedSys1, "AP");
	mgSysSmoothed->Add(SmoothedSys2, "AP");
	mgSysSmoothed->Add(SmoothedSys3, "AP");
	mgSysSmoothed->Add(SmoothedSys4, "AP");
	mgSysSmoothed->Add(SmoothedSys5, "AP");
	mgSysSmoothed->Add(SmoothedSys6, "AP");
	mgSysSmoothed->Add(SmoothedSTATerr, "AP");
	/*Sys1->Draw("hist");
	Sys2->Draw("histsame");
	Sys3->Draw("histsame");
	Sys4->Draw("histsame");
	Sys5->Draw("histsame");
	Sys6->Draw("histsame");*/
	mgSysSmoothed->Draw("AP");
	legend->Draw("same");
	mgSysSmoothed->SetTitle("Smoothed Uncertainty Contributions");
	mgSysSmoothed->GetXaxis()->SetTitle(xaxistitle.c_str());
	mgSysSmoothed->GetYaxis()->SetTitle("Uncertainty #deltaA_{#Lambda}");
	mgSysSmoothed->GetYaxis()->SetTitleSize(0.06);
	mgSysSmoothed->GetXaxis()->SetTitleSize(0.06);
	outputfile.cd();
	c3->Update();
	c3->SetGrid();
	c3->Write();
	...
	//LambdaRegDataCountincl = AnalyzeTree(varidx, new TFile(filelambdaTree.c_str(), "read"), "Lambda Reg", 1, 0, LambdaRegDataxincl, { q2bins[0],q2bins[q2entries] }, "Lambda Reg Incl Data q2 Extraction ", c3, c1, { {0.0023,3.0,1.1156} }, { {1.08,1.096,1.137,1.15} });
	//LambdaRegMCRAPGAPCountincl = AnalyzeTree(varidx, new TFile(filelambdaTreeMCRAPGAP.c_str(), "read"), "Lambda Reg", 1, 0, LambdaRegMCRAPGAPxincl, { q2bins[0],q2bins[q2entries] }, "Lambda Reg Incl MC RAPGAP q2 Extraction ", c3, c1, { {0.0019, 2.5,1.1156} }, { {1.08,1.098,1.137,1.15} });
	//LambdaRegMCDJANGOHCountincl = AnalyzeTree(varidx, new TFile(filelambdaTreeMCDJANGOH.c_str(), "read"), "Lambda Reg", 1, 0, LambdaRegMCDJANGOHxincl, { q2bins[0],q2bins[q2entries] }, "Lambda Reg Incl MC DJANGOH q2 Extraction ", c3, c1, { {0.0019, 2.5,1.1156} }, { {1.08,1.098,1.137,1.15} });

	//LambdaBarDataCountincl = AnalyzeTree(varidx, new TFile(filelambdaTree.c_str(), "read"), "Lambda Bar", 1, 0, LambdaBarDataxincl, { q2bins[0],q2bins[q2entries] }, "Lambda Bar Incl Data q2 Extraction ", c3, c1, { {0.0023,3.0,1.1156} }, { {1.08,1.096,1.137,1.15} });
	//LambdaBarMCRAPGAPCountincl = AnalyzeTree(varidx, new TFile(filelambdaTreeMCRAPGAP.c_str(), "read"), "Lambda Bar", 1, 0, LambdaBarMCRAPGAPxincl, { q2bins[0],q2bins[q2entries] }, "Lambda Bar Incl MC RAPGAP q2 Extraction ", c3, c1, { {0.0016,2.5,1.1156} }, { {1.08,1.096,1.137,1.15} });
	//LambdaBarMCDJANGOHCountincl = AnalyzeTree(varidx, new TFile(filelambdaTreeMCDJANGOH.c_str(), "read"), "Lambda Bar", 1, 0, LambdaBarMCDJANGOHxincl, { q2bins[0],q2bins[q2entries] }, "Lambda Bar Incl MC DJANGOH q2 Extraction ", c3, c1, { {0.0016,2.5,1.1156} }, { {1.08,1.096,1.137,1.15} });
	...
	//double INCMCerr = sqrt(Square((2* LambdaBarMCRAPGAPCountincl[0][0]* tempINClambareffRAPGAP* LambdaRegMCRAPGAPCountincl[0][0]/Square(LambdaBarMCRAPGAPCountincl[0][0]* tempINClamregeffRAPGAP + tempINClambareffRAPGAP* LambdaRegMCRAPGAPCountincl[0][0]))*(0.5*abs(tempINClamregeffDJANGOH- tempINClamregeffRAPGAP)))+Square((2 * LambdaBarMCRAPGAPCountincl[0][0] * tempINClambareffRAPGAP * LambdaRegMCRAPGAPCountincl[0][0] / Square(LambdaBarMCRAPGAPCountincl[0][0] * tempINClamregeffRAPGAP + tempINClambareffRAPGAP * LambdaRegMCRAPGAPCountincl[0][0])) *(0.5*abs(tempINClambareffDJANGOH- tempINClambareffRAPGAP))));
	...
	//cout << "RAPGAP Asymmetry: " << INCASYMRAPGAP << " +/- " << INCASYMstaterrRAPGAP << " (stat.)" << endl;
	//cout << "Integrated Asymmetry: " << INCASYMRAPGAP << " +/- " << INCASYMstaterrRAPGAP << "(stat.) +/- " << INCAsymmerrSYS << " (sys.)" << endl;
	...
	////runningintegratedAsym = runningintegratedAsym + yAsymm[k] * 2 * xerr[k];
	////runningintegratedAsymerror = runningintegratedAsymerror + Square(yAsymmTOTerr[k] * (2 * xerr[k]) / (q2bins[q2entries] - q2bins[0]));
	//cout << "Integrated Asym Bin by Bin: " << runningintegratedAsym << " +/- " << sqrt(runningintegratedAsymerror) << endl;
