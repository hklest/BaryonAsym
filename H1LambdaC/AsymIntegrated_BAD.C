...
vector<double> mlpfunction(TH1F* inHist, string plotname, int particlecode, TCanvas* c3, vector<double> fitparaminit, vector<double> rangedefs) { //when calling function, particle code deictates 0=lambda or 1=kaon MLP parameters
	...
	RooRealVar mu("mu", "mu", 1.1156, 1.1152, 1.1161);
	RooRealVar sigma("sigma", "sigma", fitparaminit[0], 0.0015, 0.0025);
	//RooRealVar sigma("sigma", "sigma", 0.001, 0.001, 0.005 );
	RooRealVar p3("p3", "p3", fitparaminit[1],2.0,5.0);
	...
	RooRealVar sigmadouble("sigmadouble", "sigmadouble", fitparaminit[0], 0.0015, 0.0025);
	//RooRealVar sigmadouble("sigmadouble", "sigmadouble", 0.001, 0.001, 0.005);
	RooRealVar p3double("p3double", "p3double", fitparaminit[1], 2.0, 5.0);
		...
		for (j = 0; j < binentries; j++) {
			if(s61>0){
				if (pT > 0.5 && pT < 3.5) {
					if (abs(Eta) < 1.3) {
						if (Ys > 0.1 && Ys < 0.6) {
							//if(Q2s / (Ys * 101568)>0.0004 && Q2s / (Ys * 101568) < 0.01){
								if (Q2s > 5 && Q2s < 100) {
...
//This function takes in raw tree from TTree input files and creates histograms with appropriate binning for peak extractions
//after it has all histrograms for appropriate bins, it extracts counts with mlpfunciton, output is a vector of mlpfunciton vector outputs
vector<vector<double>> AnalyzeTreeMC(int distvariableindex, TFile* Data, string TreeName, int binentries, int particlecode, vector<TH1F*> HistArray, vector<double> binsvector, string extractionReadback, TCanvas* c3, TCanvas* c1, vector<vector<double>> binparaminits, vector<vector<double>> binrangedefs) { //when calling function, particle code deictates 0=lambda or 1=kaon MLP parameters
	vector<vector<double>> ExtractionDataVector = {};
...
void AsymIntegrated(int varidx, vector<double> q2bins, double DataIntLumi06, double MCDjangoh14radgen, double MCDjangoh14nonradgen, string filekaonTree, string filelambdaTree, string filekaonTreeMC, string filelambdaTreeMCRAPGAP, string filelambdaTreeMCDJANGOH, string filegenMCTreeRAPGAP, string filegenMCTreeDJANGOH, string fileNonRadgenMCTree, string fileRadgenMCTree, string xaxistitle, vector<vector<double>> binparaminitsLambda, vector<vector<double>> binrangedefsLambda, vector<vector<double>> binparaminitsLambdaBar, vector<vector<double>> binrangedefsLambdaBar, vector<vector<double>> binparaminitsMCLambda, vector<vector<double>> binrangedefsMCLambda, vector<vector<double>> binparaminitsMCLambdaBar, vector<vector<double>> binrangedefsMCLambdaBar) {
	...
	*/
	...
	//LambdaRegDataCountincl = AnalyzeTree(varidx, new TFile(filelambdaTree.c_str(), "read"), "Lambda Reg", 1, 0, LambdaRegDataxincl, { q2bins[0],q2bins[q2entries] }, "Lambda Reg Incl Data q2 Extraction ", c3, c1, { {0.0023,3.0,1.1156} }, { {1.082,1.096,1.137,1.16} });
	...
	LambdaRegMCRAPGAPCountincl = AnalyzeTreeMC(varidx, new TFile(filelambdaTreeMCRAPGAP.c_str(), "read"), "Lambda Reg Gen", 1, 0, LambdaRegMCRAPGAPxincl, { q2bins[0],q2bins[q2entries] }, "Lambda Reg Incl MC RAPGAP q2 Extraction ", c3, c1, { {0.0019, 2.5,1.1156} }, { {1.08,1.098,1.137,1.15} });
	LambdaRegMCDJANGOHCountincl = AnalyzeTreeMC(varidx, new TFile(filelambdaTreeMCDJANGOH.c_str(), "read"), "Lambda Reg Gen", 1, 0, LambdaRegMCDJANGOHxincl, { q2bins[0],q2bins[q2entries] }, "Lambda Reg Incl MC DJANGOH q2 Extraction ", c3, c1, { {0.0019, 2.5,1.1156} }, { {1.08,1.098,1.137,1.15} });


	//LambdaBarDataCountincl = AnalyzeTree(varidx, new TFile(filelambdaTree.c_str(), "read"), "Lambda Bar", 1, 0, LambdaBarDataxincl, { q2bins[0],q2bins[q2entries] }, "Lambda Bar Incl Data q2 Extraction ", c3, c1, { {0.0023,2.5,1.1156} }, { {1.08,1.096,1.137,1.15} });
	
	//LambdaBarMCRAPGAPCountincl = AnalyzeTree(varidx, new TFile(filelambdaTreeMCRAPGAP.c_str(), "read"), "Lambda Bar", 1, 0, LambdaBarMCRAPGAPxincl, { q2bins[0],q2bins[q2entries] }, "Lambda Bar Incl MC RAPGAP q2 Extraction ", c3, c1, { {0.0016,2.5,1.1156} }, { {1.079,1.096,1.137,1.153} });
	//LambdaBarMCDJANGOHCountincl = AnalyzeTree(varidx, new TFile(filelambdaTreeMCDJANGOH.c_str(), "read"), "Lambda Bar", 1, 0, LambdaBarMCDJANGOHxincl, { q2bins[0],q2bins[q2entries] }, "Lambda Bar Incl MC DJANGOH q2 Extraction ", c3, c1, { {0.00161,3.0,1.1156} }, { {1.079,1.096,1.139,1.157} });
	LambdaBarMCRAPGAPCountincl = AnalyzeTreeMC(varidx, new TFile(filelambdaTreeMCRAPGAP.c_str(), "read"), "Lambda Bar Gen", 1, 0, LambdaBarMCRAPGAPxincl, { q2bins[0],q2bins[q2entries] }, "Lambda Bar Incl MC RAPGAP q2 Extraction ", c3, c1, { {0.0015,2.5,1.1156} }, { {1.079,1.096,1.137,1.153} });
	LambdaBarMCDJANGOHCountincl = AnalyzeTreeMC(varidx, new TFile(filelambdaTreeMCDJANGOH.c_str(), "read"), "Lambda Bar Gen", 1, 0, LambdaBarMCDJANGOHxincl, { q2bins[0],q2bins[q2entries] }, "Lambda Bar Incl MC DJANGOH q2 Extraction ", c3, c1, { {0.00155,2.5,1.1156} }, { {1.079,1.096,1.137,1.155} });
	...
	double INCAsymmerrSYS = sqrt(Square(INCMCerr) + Square(INCextractionrange) + Square(INCrawextraction) + Square(INCalternativeBGfit) + Square(INCdetectorinterror));
	...
	cout << "Total Data Lambda Reg Counts: " << LambdaRegDataCountincl[0][0] << endl;
	cout << "Total Data Lambda Bar Counts: " << LambdaBarDataCountincl[0][0] << endl;
