...
root_files_location="/pnfs/desy.de/dphep/online/h1/data/oo-4.0/07med/"
...
for ((i = start_number; i < end_number; i++)); do
    ...
    output_file="/data/dust/group/h1/klesthen/BaryonAsym/H1LambdaC/Steering/2007med/SelectedEvents07med_${i}.steer"
    printf "${root_files_location}${hat_files_prefix}${file_number}.root \n"
    printf "$output_file\n"
    # Check if the file exists
    if [ -e "$file_path" ]; then
        echo "H1SteerTree() {" > "$output_file"
        echo "   //2007 Med Data" >> "$output_file"
