...
main(int argc, char* argv[])
{
    ...
    H1FloatPtr epz("Epz");
    //H1FloatPtr Triggers("fIl1ac[61]"); //S61
...
//Defining Tree Branches
    ...
    EventTree->Branch("fRunType", &fRunType, "fRunType/D");
    ...
    EventTree->Branch("s61", &s61, "s61/I");
    EventTree->Branch("s1", &s1, "s1/I");
    EventTree->Branch("s74", &s74, "s74/I");
    EventTree->Branch("s75", &s75, "s75/I");
    EventTree->Branch("s67", &s67, "s67/I");
    EventTree->Branch("beamCharge", &beamCharge, "beamCharge/I");
    EventTree->Branch("TrType", &TrType, "TrType/I");
    EventTree->Branch("TrackCheck", &TrackCheck, "TrackCheck/I");
    EventTree->Branch("measuredCharge", &measuredCharge, "measuredCharge/D");
    EventTree->Branch("epT", &epT, "epT/D");
    ...
    while (H1Tree::Instance()->Next()) {
        ...
        //cout << "S61 Trigger: " << gH1Calc->Trig()->GetL1ac(61) << endl;
        //cout << "S74 Trigger: " << gH1Calc->Trig()->GetL1ac(74) << endl;
        //cout << "S1 Trigger: " << gH1Calc->Trig()->GetL1ac(1) << endl;
       scatelCount = 0;
        ...
        for (Int_t loop = 0; loop < PartEm.GetEntries(); loop++) {
            ...
            if (PartEm[loop]->IsScatElec() == true) { 
                ...
                measuredCharge = PartEm[loop]->GetCharge();
                epT = PartEm[loop]->GetPt();
                TrType = PartEm[loop]->GetTrType();
                fElecID = loop;
            }
        }
        ...
        if (*vtxZ<=-30 || *vtxZ>=30 || *vtxZ==0) { continue; } //-30 < Z_vertex [cm] < 30
        zvtxEvents++;
...
//        //Alignment fSpacAlignment;
    ...
    //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        if (*ys <= 0.01 || *ys >= 0.6) { continue; } // 0.1 < y < 0.6
        yEvents++;
    ...
    //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
      ...
      if (*ys < 0.6 && *ys>0.01 && *q2s > 5 && *q2s < 100 && *eE>10 && *epz > 35 && *epz < 70 && *vtxZ < 30 && *vtxZ > -30)    {
        //cout << "E: " << *Echeck << endl;
        //cout << "EPZ: " << *epz << endl;
        ...
        //cout << "EPZ calc: " <<  ( * ElecE) + abs((*ElecE)*cos(*ElecTheta)) + (*hfsE) - abs((*hfsE) * cos(*hfsTheta)) << endl;
          ...
          //cout << gH1Constants->GetElectronBeamCharge() << endl;
          //cout << "Run Period                          = ";
          if (RunPeriod == H1Constants::eEminus0405) {
              beamCharge = -1;
              //cout << "e- 04/05" << endl;
          }
          else if (RunPeriod == H1Constants::eEminus06) {
              beamCharge = -1;
              //cout << "e- 06" << endl;
          }
          else if (RunPeriod == H1Constants::eEplus0607) {
              beamCharge = 1;
              //cout << "e+ 06/07" << endl;
          }
          else { 
          ...
          TrackCheck = myElecCut.elecCutTrack(PartEm[fElecID], RunNumber); // if this =1, then there is an associated track to use, so now we must check the charge

         /* if (beamCharge != PartEm[fElecID]->GetCharge()) {
              cout << "Track Check: " << TrackCheck << endl;
          }*/
          ...
          sel++;
         if(s61>0){ list->Enter(); }//enter event into our event list as long as S61 is triggered, this was not enforced in previous step so that we can calculate the trigger efficiency from EventTree data
      }
    }
