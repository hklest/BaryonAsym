...
double fTaylorLambda(double* x, double* par)
{
	if (reject && x[0] > 1.0976 && x[0] < 1.14) {
		TF1::RejectPoint();
		return 0;
...
double fPolyLambda(double* x, double* par)
{
	if (reject && x[0] > 1.103 && x[0] < 1.13) {
		TF1::RejectPoint();
		return 0;
	}

	//exclude secondary lambda peak that is seen only in the lowest pT bin
...
double fline(double* x, double* par, double leftboundexclude, double rightboundexclude)
{
	if (reject && x[0] > leftboundexclude && x[0] < rightboundexclude) {
		TF1::RejectPoint();
		return 0;
	}
...
double flineVal(double x, double* par)
{
	return par[0] + par[1] * x;
}

//Polynomial Root Fit
vector<double> mlpfunctionL(TH1F* inHist, string plotname, int particlecode, TCanvas* c3, vector<double> fitparaminit, vector<double> rangedefs) { //when calling function, particle code deictates 0=lambda or 1=kaon MLP parameters
	...
	TF1* f1 = new TF1("f1", flineLambda, 1.0878, 1.16, 2);
	...
	//Fit only the BG regions by enabling reject tag
	reject = true;
...
//Fitting using multi layer perceptron method
vector<double> mlpfunctionMLP(TH1F* inHist, string plotname, int particlecode, TCanvas* c3, TCanvas* c1, vector<double> fitparaminit, vector<double> rangedefs) { //when calling function, particle code deictates 0=lambda or 1=kaon MLP parameters
...
//Fitting only the BG here with RooFit
vector<double> mlpfunctionBGthisdoesnotwork(TH1F* inHist, string plotname, int particlecode, TCanvas* c3, TCanvas* c1, vector<double> fitparaminit, vector<double> rangedefs) { //when calling function, particle code deictates 0=lambda or 1=kaon MLP parameters
	...
	RooRealVar mLam("mLam", "mLam", x1, x4); //observable
	...
	bg.fitTo(RooFitDataHist, RooFit::Extended(kTRUE), RooFit::Range("left,right"), RooFit::Save());
	//bg.fitTo(RooFitDataHist, RooFit::Range("left,right"), RooFit::Save());
	RooPlot* RooFitPlot = mLam.frame();
	RooFitDataHist.plotOn(RooFitPlot);

	bg.plotOn(RooFitPlot, RooFit::Range("full"), RooFit::LineColor(kBlue));
	...
	TH2F* bgcheckHist = (TH2F*)inHist->Clone();
	bgcheckHist->Reset("ICESM");
	bgcheckHist->SetLineColor(kGreen);

	TH2F* sigcheckHist = (TH2F*)inHist->Clone();
	sigcheckHist->Reset("ICESM");
	sigcheckHist->SetLineColor(kOrange);

	TH2F* sumcheckHist = (TH2F*)inHist->Clone();
	sumcheckHist->Reset("ICESM");
	sumcheckHist->SetLineColor(kGreen);

	TH2F* xaxis = (TH2F*)inHist->Clone();
	xaxis->Reset("ICESM");
	xaxis->SetLineColor(kBlack);
	...
	for (int i = 1; i < fgHist->GetNbinsX() + 1; i++) {
		inputs[0] = ((TAxis*)fgHist->GetXaxis())->GetBinCenter(i);
		if ((inputs[0] >= x1 && inputs[0] < x2) || (inputs[0] >= x3 && inputs[0] < x4)) {
			ntotcheck = ntotcheck + fgHist->GetBinContent(i);
		}
	}
	std::unique_ptr<RooAbsReal> ig2{ bg.createIntegral(mLam, RooFit::NormSet(mLam), RooFit::Range("signal")) };
	//double ntot = RooFitDataHist.sumEntries(0, "left") + RooFitDataHist.sumEntries(0, "right");//number of events in this bin
	//double ntot = RooFitDataHist.sumEntries(0, "bin");//number of events in this bin
	double ntot = RooFitDataHist.sumEntries();
	double bgyielddvalue = ig2->getVal() * ntot;
	...
	double tempsignalyieldvalue = 0;
	double tempsumyieldvalue = 0;
	double tempbgyieldvalue = 0;
	...
	//LEGEND
	//TLegend* legend = new TLegend(0.5513, 0.4144, 0.888, 0.8823);
	//legend->SetHeader(Form("#splitline{Count: %.2f #pm %.2f (stat.)}{Fit:}", signalyieldvalue, countstaterror)); // option "C" allows to center the header
	...
	return { totalcountwithbg - bgyielddvalue ,countstaterror, bgyielddvalue, totalcountwithbg - bgyielddvalue };
};
...
vector<double> mlpfunction(TH1F* inHist, string plotname, int particlecode, TCanvas* c3, vector<double> fitparaminit, vector<double> rangedefs) { //when calling function, particle code deictates 0=lambda or 1=kaon MLP parameters
	...
	// Background PDF
	//RooRealVar a0("a0", "a0", 10., .0, 1.0e6);
	RooRealVar b1("b1", "b1", 0.5,-10000,1000000);
	RooRealVar b2("b2", "b2", 0.5, -10000, 1000000);
	RooRealVar b3("b3", "b3", -0.5, -10000, 1000000);
	RooRealVar b4("b4", "b4", 0.5, -10000, 1000000);
	RooRealVar b5("b5", "b5", -0.5, -10000, 1000000);
	RooRealVar nsigalt("nsigalt", "alt # signal events", inHist->Integral(), 0, 2*inHist->Integral());
	RooRealVar nbkgalt("nbkgalt", "alt # background events", 100, 0, 2*inHist->Integral());
	...
	RooRealVar sigmadouble("sigmadouble", "sigmadouble", fitparaminit[0]);
	RooRealVar p3double("p3double", "p3double", fitparaminit[1]);
	RooRealVar nsig("nsig", "# signal events", inHist->Integral(), 0, 2 * inHist->Integral());
	RooRealVar nbkg("nbkg", "# background events", 100, 0, 2*inHist->Integral());
	RooGenericPdf bg("bg", "TMath::Power(TMath::Max(mLam-1.0782,0.),a1)*TMath::Exp(a2*(mLam-1.0782))", RooArgSet(mLam, a1, a2));
	...
	//For Students T, mu will be the center of the particle peak
	//We will initialize its value as the center of the maximum bin
	//RooRealVar mu("mu", "mu", fitparaminit[2], fitparaminit[2]-0.001, fitparaminit[2]+0.001);
	//RooRealVar mu("mu", "mu", inHist->GetBinCenter(inHist->GetMaximumBin()));
	...
	// Signal+Bg PDF
	////////////////////////////////////////////////////////////////////////////////////
	//RooRealVar nsig("nsig", "# signal events", 1.0, 0, 1000000000000);
	//RooRealVar nbkg("nbkg", "# background events", 1.0, 0, 100000000000);
	//RooAddPdf sum("sum", "sigf+bg", RooArgList(sigf, bg), RooArgList(nsig, nbkg));
	...
	mLam.setRange("altfitrange", s1-0.01, s2+0.015);
	...
	//Do linear fit in root
	//TF1* f1 = new TF1("f1", fTaylorLambda, s1-0.01, s2+0.01, 3);
	...
	//Fit only the BG regions by enabling reject tag
	//reject = true;
	//inHist->Fit("f1", "R0", "0", 1.0878, 1.16);
	//reject = false;
	...
	for (int i = 1; i < (int)inHist->GetNbinsX() + 1; i++) {
		...
		if ((bcx > x1 && bcx < x2) || (bcx > x3 && bcx < x4))
			//ofl << bcx << "\t" << inHist->GetBinContent(i) << endl;
			ntp_bg->Fill(bcx, inHist->GetBinContent(i));
	}
	...
	sigcheckHist->Reset("ICESM");
	sigcheckHist->SetLineColor(kRed);
	...
	legend->SetHeader(Form("#splitline{Count: %.2f #pm %.2f (stat.)#pm %.2f (Fit err.)}{Percent Diff from Alt BG: %.2f}", signalyieldvalue, countstaterror, signalyieldvalueerror, 100*abs(signalyieldvalue- altsignalyieldvalue)/ signalyieldvalue)); // option "C" allows to center the header
	legend->AddEntry("RooFitDataHist", "Data");
	//legend->AddEntry(fitHist, "Fit Accross Data BG", "l");
	legend->AddEntry(sumcheckHist, "Signal+BG Fit");
	//legend->AddEntry(extractionbound1line, "Signal Extraction Range", "l");
	//legend->AddEntry(rightfitbound1line, "Left and Right BG Fit Ranges", "l");
	legend->AddEntry(sigcheckHist, "Signal");
	legend->Draw("same");
	//END LEGEND
...
//Main peak extraction, composite fit with LINEAR background in RooFit
vector<double> mlpfunctionLINROO(TH1F* inHist, string plotname, int particlecode, TCanvas* c3, vector<double> fitparaminit, vector<double> rangedefs) { //when calling function, particle code deictates 0=lambda or 1=kaon MLP parameters
	...
	RooRealVar nbkg("nbkg", "# background events", 1.0, 0, 100000000000);
	//RooGenericPdf bg("bg", "TMath::Power(TMath::Max(mLam-1.0782,0.),a1)*TMath::Exp(a2*(mLam-1.0782))", RooArgSet(mLam, a1, a2));
	RooGenericPdf bg("bg", "a1*mLam + a2", RooArgSet(mLam, a1, a2));
	RooGenericPdf sigf("sigf", "sigf", "TMath::Gamma((p3+1)/2)/TMath::Gamma(p3/2)/TMath::Sqrt(TMath::Pi()*p3)*TMath::Power(1.+((mLam-mu)/sigma)*((mLam-mu)/sigma)/p3,-(p3+1)/2)", RooArgSet(mLam, mu, p3, sigma));
	RooAddPdf sum("sum", "sigf+bg", RooArgList(sigf, bg), RooArgList(nsig, nbkg));
	// Signal PDF
	//Students T Distribution
	...
	//LEGEND
	TLegend* legend = new TLegend(0.5513, 0.4144, 0.888, 0.8823);
	//legend->SetHeader(Form("#splitline{Count: %.2f #pm %.2f (stat.)#pm %.2f (Fit err.)}{Percent Diff Alt BG: %.2f}", signalyieldvalue, countstaterror, signalyieldvalueerror, ); // option "C" allows to center the header
	legend->AddEntry("RooFitDataHist", "Data");
	//legend->AddEntry(fitHist, "Fit Accross Data BG", "l");
	legend->AddEntry(sumcheckHist, "Signal+BG Fit");
	//legend->AddEntry(extractionbound1line, "Signal Extraction Range", "l");
	...
	*RatioHist = (*inHist) / (*sumcheckHist);
	...
	//return { signalyieldvalue ,countstaterror, bgyielddvalue, signalyieldvalueerror, totalcountwithbg - bgyielddvalue };
	//return { SIGNALINT ,countstaterror, bgyielddvalue, signalyieldvalueerror, totalcountwithbg - bgyielddvalue };
	//return { runningrawsignalsubtraction ,countstaterror, bgyielddvalue, signalyieldvalueerror, totalcountwithbg - bgyielddvalue };
	return { runningrawsignalsubtraction ,countstaterror, bgyielddvalue, signalyieldvalueerror, totalcountwithbg - bgyielddvalue };
...
//Modified BG peak extraction
vector<double> mlpfunctionmod(TH1F* inHist, string plotname, int particlecode, TCanvas* c3, TCanvas* c1, vector<double> fitparaminit, vector<double> rangedefs) { //when calling function, particle code deictates 0=lambda or 1=kaon MLP parameters
	...
	RooRealVar a1("a1", "a1", 0.5, 0., 5.);
	RooRealVar a2("a2", "a2", 0.0, -100., 100.);
	RooGenericPdf bg("bg", "TMath::Power(TMath::Max(mLam-1.0782,0.),a1)", RooArgSet(mLam, a1));
	...
	for (int i = 1; i < fgHist->GetNbinsX() + 1; i++) {
		...
		if ((inputs[0] >= s1 && inputs[0] < s2)) {
			...
			tempsignalyieldvalue = tempsignalyield.getVal();
			tempsignalyieldvalueerror = tempsignalyield.getPropagatedError(*result, mLam);
			cout << "Error Check Bin: " << tempsignalyieldvalue << " +/- " << tempsignalyieldvalueerror << endl;

			checkHist->SetBinContent(i, tempsignalyieldvalue);
			checkHist->SetBinError(i, tempsignalyieldvalueerror);
	...
	//return { signalyieldvalue ,countstaterror, bgyielddvalue, signalyieldvalueerror, totalcountwithbg - bgyielddvalue };
	return { totalcountwithbg - bgyielddvalue ,countstaterror, bgyielddvalue, signalyieldvalueerror, totalcountwithbg - bgyielddvalue };
};



//Linear BG function peak extraction
vector<double> linearfunction(TH1F* inHist, string plotname, int particlecode, TCanvas* c3, TCanvas* c1, vector<double> fitparaminit, vector<double> rangedefs) { //when calling function, particle code deictates 0=lambda or 1=kaon MLP parameters
	...
	RooRealVar mLam("mLam", "mLam", x1, x4); //observable
	mLam.setRange("full", x1, x4);
	mLam.setRange("left", x2, s1);
	mLam.setRange("right", x3, 1.14);
	mLam.setRange("signal", s1, s2);
	mLam.setRange("linearfitrange", x2, 1.14);
	//Composite
	// Background PDF
	...
	RooFitResult* result = sum.fitTo(RooFitDataHist, RooFit::Range("linearfitrange"), RooFit::Save());
...
void Asym(int varidx, vector<double> q2bins, double DataIntLumi06, double MCDjangoh14radgen, double MCDjangoh14nonradgen, string filekaonTree, string filelambdaTree, string filekaonTreeMC, string filelambdaTreeMCRAPGAP, string filelambdaTreeMCDJANGOH, string filegenMCTreeRAPGAP, string filegenMCTreeDJANGOH, string fileNonRadgenMCTree, string fileRadgenMCTree, string xaxistitle, vector<vector<double>> binparaminitsLambda, vector<vector<double>> binrangedefsLambda, vector<vector<double>> binparaminitsLambdaBar, vector<vector<double>> binrangedefsLambdaBar, vector<vector<double>> binparaminitsMCLambda, vector<vector<double>> binrangedefsMCLambda, vector<vector<double>> binparaminitsMCLambdaBar, vector<vector<double>> binrangedefsMCLambdaBar) {
	...
	LambdaAsym->SetLineColor(kBlack);
	LambdaAsymSYS->SetLineColor(kRed);
	legend->AddEntry(LambdaAsym, "stat.", "l");
	legend->AddEntry(LambdaAsymSYS, "sys.", "l");
	...
	mgSys->Draw("AP");
	legend->Draw("same");
	mgSys->SetTitle("Systematic Uncertainty Contributions");
	...
	TLine xequalsyline = TLine(0, 0, 10, 10);
	...
	mgScatter->Draw("AP");
	mgScatter->GetXaxis()->SetLimits(0., 10.);
	mgScatter->GetYaxis()->SetLimits(0., 10.);
	legend->Draw("same");
	...
	LambdaRegMCRAPGAPCountincl = AnalyzeTree(varidx, new TFile(filelambdaTreeMCRAPGAP.c_str(), "read"), "Lambda Reg", 1, 0, LambdaRegMCRAPGAPxincl, { q2bins[0],q2bins[q2entries] }, "Lambda Reg Incl MC RAPGAP q2 Extraction ", c3, c1, { {0.0017,2.0,1.1156} }, { {1.08,1.096,1.137,1.15} });
	LambdaRegMCDJANGOHCountincl = AnalyzeTree(varidx, new TFile(filelambdaTreeMCDJANGOH.c_str(), "read"), "Lambda Reg", 1, 0, LambdaRegMCDJANGOHxincl, { q2bins[0],q2bins[q2entries] }, "Lambda Reg Incl MC DJANGOH q2 Extraction ", c3, c1, { {0.0018,2.0,1.1156} }, { {1.08,1.096,1.137,1.15} });
	...
	LambdaBarMCRAPGAPCountincl = AnalyzeTree(varidx, new TFile(filelambdaTreeMCRAPGAP.c_str(), "read"), "Lambda Bar", 1, 0, LambdaBarMCRAPGAPxincl, { q2bins[0],q2bins[q2entries] }, "Lambda Bar Incl MC RAPGAP q2 Extraction ", c3, c1, { {0.0017,2.0,1.1156} }, { {1.08,1.096,1.137,1.15} });
	LambdaBarMCDJANGOHCountincl = AnalyzeTree(varidx, new TFile(filelambdaTreeMCDJANGOH.c_str(), "read"), "Lambda Bar", 1, 0, LambdaBarMCDJANGOHxincl, { q2bins[0],q2bins[q2entries] }, "Lambda Bar Incl MC DJANGOH q2 Extraction ", c3, c1, { {0.0018,2.0,1.1156} }, { {1.08,1.096,1.137,1.15} });
