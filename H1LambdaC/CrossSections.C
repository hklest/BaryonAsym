...
vector<double> mlpfunctionLambda(TH1F* inHist, string plotname, int particlecode, TCanvas* c3, vector<double> fitparaminit, vector<double> rangedefs) { //when calling function, particle code deictates 0=lambda or 1=kaon MLP parameters
	...
	//double FWHM = 2 * sqrt(2*p3.getVal()*(pow(2,2/(p3.getVal()+1))-1));
	...
	//double FWHMalt = 2 * sqrt(2 * p3double.getVal() * (pow(2, 2 / (p3double.getVal() + 1)) - 1));
...
int PlotData(string plotcanvasname, string plottitle, string xaxistitle, string yaxistitle, double ylowbound, double yupbound, TGraphErrors* cs, int binentries, double X[binentries], double y[binentries], double xerr[binentries], double y1err[binentries], double dylow[binentries], double dyup[binentries], double dylow2[binentries], double dyup2[binentries]) { //when calling function, particle code deictates 0=lambda or 1=kaon MLP parameters
	...
	c5->SetLeftMargin(0.15); //root default is 0.1
	...
	mgcs->GetYaxis()->SetTitleSize(0.04);
...
void CrossSections(int varidx, vector<double> q2bins, double DataIntLumi06, double MCDjangoh14radgen, double MCDjangoh14nonradgen, string filekaonTree, string filelambdaTree, string filekaonTreeMC, string filelambdaTreeMCRAPGAP, string filelambdaTreeMCDJANGOH, string filegenMCTreeRAPGAP, string filegenMCTreeDJANGOH, string fileNonRadgenMCTree, string fileRadgenMCTree, string xaxistitle, vector<vector<double>> binparaminitsLambda, vector<vector<double>> binrangedefsLambda, vector<vector<double>> binparaminitsKaon, vector<vector<double>> binrangedefsKaon, vector<vector<double>> binparaminitsMCLambda, vector<vector<double>> binrangedefsMCLambda, vector<vector<double>> binparaminitsMCKaon, vector<vector<double>> binrangedefsMCKaon) {
	...
	double lambdatoteffplot[q2entries] = {};
	double kaoneffplot[q2entries] = {};
	double lambdatoteffploterr[q2entries] = {};
	double kaoneffploterr[q2entries] = {};
	...
	for (int k = 0; k < q2entries; k++) {
		...
		lambdatoteffplot[k] = templameff;
		lambdatoteffploterr[k] = templamefferr;
		...
		cout << "Lambda Cross Section in Bin " << k + 1 << " : " << y1[k] << " +/- " << y1err[k] << " (stat.) " << endl;
	}
	...
	PlotData("Lambda Cross Section vs q2", "d#sigma(ep #rightarrow e' [#Lambda + #bar{#Lambda}] X)/dx", xaxistitle.c_str(), "d#sigma_{vis}(ep #rightarrow e' (#Lambda+#bar{#Lambda}) X)/dx [nb]",0,4.2, cs, q2entries, xvals, y, xerr, y1err, dylow, dyup, dylow2, dyup2);
	for (int k = 0; k < q2entries; k++) {
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	for (int k = 0; k < q2entries; k++) {
		...
		kaoneffplot[k] = tempk0eff;
		kaoneffploterr[k] = tempk0efferr;


		cout << "K0s Cross Section in Bin " << k + 1 << " : " << y1[k] << " +/- " << y1err[k] << " (stat.) " << endl;
	...
	TGraphAsymmErrors* LambdaTotRAPGAPeff = new TGraphAsymmErrors(q2entries, xvals, lambdatoteffplot, xerr, xerr, lambdatoteffploterr, lambdatoteffploterr);
	TGraphAsymmErrors* KaonRAPGAPeff = new TGraphAsymmErrors(q2entries, xvals, kaoneffplot, xerr, xerr, kaoneffploterr, kaoneffploterr);
	
	LambdaTotRAPGAPeff->SetLineColor(kBlack);
	KaonRAPGAPeff->SetLineColor(kRed);

	LambdaTotRAPGAPeff->SetMarkerColor(kBlack);
	KaonRAPGAPeff->SetMarkerColor(kRed);

	LambdaTotRAPGAPeff->SetLineWidth(2);
	KaonRAPGAPeff->SetLineWidth(2);

	//LambdaTotRAPGAPeff->SetMarkerStyle(21);
	//KaonRAPGAPeff->SetMarkerStyle(21);

	LambdaTotRAPGAPeff->SetMarkerSize(1.0);
	KaonRAPGAPeff->SetMarkerSize(1.0);

	mgeff->Add(LambdaTotRAPGAPeff, "AP");
	mgeff->Add(KaonRAPGAPeff, "AP");

	TLegend* legend = new TLegend(0.5513, 0.4144, 0.888, 0.8823);
	legend->AddEntry(LambdaTotRAPGAPeff, "RAPGAP #epsilon_{#Lambda+#bar{#Lambda}}", "l");
	legend->AddEntry(KaonRAPGAPeff, "RAPGAP #epsilon_{K^{0}_{s}}", "l");
	...
	cradcorr->SetLeftMargin(0.15); //root default is 0.1
	cradcorr->SetRightMargin(0.05);
	cradcorr->SetTopMargin(0.1);
	cradcorr->SetBottomMargin(0.15);
	cradcorr->SetGrid();
	cradcorr->SetName("Radiaitve Corrections");
	TMultiGraph* mgradcorr = new TMultiGraph();
	...
	//auto effbar = new TGraphErrors(arraysize, X, effbarplot, xerr, erreffbarplot);
	radcorrlambda->SetMarkerColor(kBlack);
	radcorrlambda->SetLineColor(kBlack);
	radcorrlambda->SetMarkerSize(0.01);
	radcorrk0->SetMarkerColor(kRed);
	radcorrk0->SetLineColor(kRed);
	radcorrk0->SetMarkerSize(0.01);

	radcorrlambda->SetLineWidth(2);
	radcorrk0->SetLineWidth(2);


	mgradcorr->Add(radcorrlambda, "AP");
	mgradcorr->Add(radcorrk0, "AP");

	mgradcorr->SetTitle("Radiative Corrections");
	mgradcorr->GetXaxis()->SetTitle(xaxistitle.c_str());
	mgradcorr->GetYaxis()->SetTitle("#delta_{QED} [%]");
	mgradcorr->GetYaxis()->SetTitleSize(0.06);
	mgradcorr->GetXaxis()->SetTitleSize(0.06);
	...
	PlotK0CSData("K0 Cross Section vs q2", "d#sigma_{vis}(ep #rightarrow e' K^{0}_{S} X)/dx", xaxistitle.c_str(), "d#sigma_{vis}(ep #rightarrow e' K^{0}_{S} X)/dx [nb]",0,10.5, cs3, q2entries, xvals, y, xerr, y1err, dylow, dyup, dylow2, dyup2);
	for (int k = 0; k < q2entries; k++) {
	...
	for (int k = 0; k < q2entries; k++) {
		...
		cout << "Lambda/K0s Ratio in Bin " << k + 1 << " : " << y1[k] << " +/- " << y1err[k] << " (stat.) " << endl;
	}

	auto cs4 = new TGraphErrors(q2entries, xvals, y1, xerr, y1err);
	PlotData("Lambda Over K0 Cross Section vs q2", "[d#sigma(ep #rightarrow e' #Lambda X)]/[d#sigma(ep #rightarrow e' K^{0}_{S} X)]", xaxistitle.c_str(), "[d#sigma_{vis}(ep #rightarrow e' (#Lambda+#bar{#Lambda}) X)] / [d#sigma_{vis}(ep #rightarrow e' K^{0}_{S} X)]",0,0.51, cs4, q2entries, xvals, y, xerr, y1err, dylow, dyup, dylow2, dyup2);
	...
	//LambdaRegDataCountincl = AnalyzeTree(varidx, new TFile(filelambdaTree.c_str(), "read"), "Lambda Reg", 1, 0, LambdaRegDataxincl, { q2bins[0],q2bins[q2entries] }, "Lambda Reg Incl Data q2 Extraction ", c3, c1, { {0.00205,3.0,1.1156} }, { {1.083,1.1,1.137,1.16} });
	//LambdaRegMCCountincl = AnalyzeTree(varidx, new TFile(filelambdaTreeMCRAPGAP.c_str(), "read"), "Lambda Reg", 1, 0, LambdaRegMCxincl, { q2bins[0],q2bins[q2entries] }, "Lambda Reg Incl MC q2 Extraction ", c3, c1, { {0.00205,3.0,1.1156} }, { {1.083,1.1,1.137,1.16} });

	//LambdaBarDataCountincl = AnalyzeTree(varidx, new TFile(filelambdaTree.c_str(), "read"), "Lambda Bar", 1, 0, LambdaBarDataxincl, { q2bins[0],q2bins[q2entries] }, "Lambda Bar Incl Data q2 Extraction ", c3, c1, { {0.00205,3.0,1.1156} }, { {1.083,1.1,1.137,1.16} });
	//LambdaBarMCCountincl = AnalyzeTree(varidx, new TFile(filelambdaTreeMCRAPGAP.c_str(), "read"), "Lambda Bar", 1, 0, LambdaBarMCxincl, { q2bins[0],q2bins[q2entries] }, "Lambda Bar Incl MC q2 Extraction ", c3, c1, { {0.00205,3.0,1.1156} }, { {1.083,1.1,1.137,1.16} });
	...
	KaonMCCountincl = AnalyzeTree(varidx, new TFile(filekaonTreeMC.c_str(), "read"), "K0", 1, 1, KaonMCxincl, { q2bins[0],q2bins[q2entries] }, "Kaon Incl MC q2 Extraction ", c3, c1, { {0.005,3.0,0.49761} }, { {0.35,0.4,0.6,0.67} });
	...
	//LambdaRegMCCountincl = AnalyzeTree(new TFile(filelambdaTreeMCRAPGAP.c_str(), "read"), "Lambda Reg", 1, 0, LambdaRegMCxincl, { 10,q2bins[q2entries] }, "Lambda Reg Incl MC q2 Extraction ", c3, c1);
	...
	//LambdaBarMCCountincl = AnalyzeTree(new TFile(filelambdaTreeMCRAPGAP.c_str(), "read"), "Lambda Bar", 1, 0, LambdaBarMCxincl, { 10,q2bins[q2entries] }, "Lambda Bar Incl MC q2 Extraction ", c3, c1);
	...
	//LambdaRegGenCountincl = AnalyzeGenTree(varidx, new TFile(filegenMCTreeRAPGAP.c_str(), "read"), "Lambda Reg Channel", 1, { q2bins[0],q2bins[q2entries] });
	//LambdaBarGenCountincl = AnalyzeGenTree(varidx, new TFile(filegenMCTreeRAPGAP.c_str(), "read"), "Lambda Bar Channel", 1, { q2bins[0],q2bins[q2entries] });
