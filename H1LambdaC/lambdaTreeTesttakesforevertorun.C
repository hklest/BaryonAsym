...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        vector<vector<double>> deltaInvMasses = {};
        vector<double> MinDeltaInvMasses = {};
        vector<int> NewLambdaIndices = {};
        int rejectedindexcount = 0;
        ...
        if (LambdaIndices.size() > 1 && protontrackcheck.size() != 0) {
            ...
            for (Int_t i = 0; i < protontrackcheck.size(); i++) {
                ...
                for (Int_t j = 0; j < protontrackcheck[i].size(); j++) {
                    ...
                    deltaInvMasses[i].push_back(abs(LambdaInvMasses[protontrackcheck[i][j]] - mLambda));
                }
            }
            ...
            for (Int_t i = 0; i < protontrackcheck.size(); i++) {
                MinDeltaInvMasses.push_back(minfinder(deltaInvMasses[i]));
            }

            for (Int_t i = 0; i < protontrackcheck.size(); i++) {
                //cout << "Index of Good Proton Duplicate " << i + 1 << " of " << protontrackcheck.size() << ":" << endl;
                //cout << LambdaIndices[protontrackcheck[i][MinDeltaInvMasses[i]]] << endl;
                
                for (Int_t j = 0; j < protontrackcheck[i].size(); j++) {
                    if (LambdaIndices[protontrackcheck[i][j]] != LambdaIndices[protontrackcheck[i][MinDeltaInvMasses[i]]]) {
        ...
        //Add similar if loop code for pion track duplicates here, will be exactly the same but just need to add one extra check that makes sure the pion I select doesn't use of the the rejected lambda indices
        ...
        for (Int_t a = 0; a < LambdaIndices.size(); a++) {
            ...
            const H1CentralFittedV0* v0 = LambdaCand[LambdaIndices[a]]->GetV0();
            pT = v0->GetPt();
        ...
        if (passedinitialcutscount - RejectedLambdaIndices.size() != passedallcutscount) { cout << "Bad Count" << endl; }
