...
int Plot(TFile* output,double lowerbinrange, double upperbinrange, double bins, int variable, string canvasname, string plottitle, string xtitle, string yaxis) {
	...
	//for (m = 0; m < 10; m++) {
		...
		if (Q2s > 5 && Q2s < 100 && Ys>0.1 && Ys < 0.6) {
			if (s61 > 0) {
				if (variable == 0) {
					//DataEventTree->GetEntry(m);
	...
	//pad1->SetGridx();
	if (variable == 14) {
		DataHist->GetXaxis()->SetRangeUser(0.0001, 0.01);
		PYTHIAHist->GetXaxis()->SetRangeUser(0.0001, 0.01);
		pad1->SetLogx(1);
	...
	TAxis* axisPYTHIA = PYTHIAHist->GetYaxis();
	axisPYTHIA->ChangeLabel(1, -1, -1, -1, -1, -1, " ");
	...
	PYTHIAHist->GetYaxis()->SetTitleOffset(2.00);
	PYTHIAHist->GetYaxis()->SetAxisColor(kGreen+2);
	PYTHIAHist->GetYaxis()->SetLabelColor(kGreen+2);
	PYTHIAHist->GetYaxis()->SetTitleFont(43);
	PYTHIAHist->GetYaxis()->SetTitleSize(20);
	PYTHIAHist->GetYaxis()->SetLabelFont(43);
	PYTHIAHist->GetYaxis()->SetLabelSize(15);
	...
	PYTHIAHist->Scale(1 / PYTHIALumi);
	
	DataHist->Draw("e");
	//RAPGAPHist->Draw("histsame");
	//DJANGOHHist->Draw("histsame");

	//Setting up pad for other axis
	canvas->cd();
	TPad* padR = new TPad("padR", "padR", 0, 0.35, 1, 1.0);
	padR->SetBottomMargin(0.01);
	padR->SetLeftMargin(0.17);
	...
	//Scaling PYTHIA hist to have its own scale 
	TH1F* PYTHIAsave = (TH1F*)PYTHIAHist->Clone(); //clone before scaling to use for the Ratio Plot
	Float_t rightmax = 1.1 * PYTHIAHist->GetMaximum();
	Float_t scale = gPad->GetUymax() / rightmax;
	PYTHIAHist->Scale(1);
	PYTHIAHist->GetXaxis()->SetLabelSize(0);
	PYTHIAHist->SetMarkerColor(kGreen+2);
	PYTHIAHist->Draw("histY+][");
	//Drawing a separate Y axis for the scaled Pythia plot
	//TGaxis* newaxis = new TGaxis(gPad->GetUxmax(), gPad->GetUymin(), gPad->GetUxmax(), gPad->GetUymax(), 0, rightmax, 510, "+L");
	//newaxis->SetLineColor(kGreen);
	//newaxis->SetLabelColor(kGreen);
	//newaxis->Draw();

	//DataHist->GetXaxis()->SetLabelOffset(999);
	DataHist->GetXaxis()->SetLabelSize(0);
	//DataHist->GetYaxis()->SetLabelOffset(0.1);
	//PYTHIAHist->GetYaxis()->SetLabelOffset(0.1);
	...
	*PYTHIARatioHist = (*PYTHIAsave) / (*DataHist);
	...
	PYTHIARatioHist->SetMarkerColor(kGreen+2);

	if (variable == 14) {
		PYTHIARatioHist->GetXaxis()->SetRangeUser(0.0001, 0.01);
		

	}

	DataRatioHist->SetLineWidth(1);
	//DataRatioHist->GetYaxis()->SetMaxDigits(4);

	TAxis* axis2 = DataRatioHist->GetYaxis();
	axis2->ChangeLabel(1, -1, -1, -1, -1, -1, " ");

	//TH1F* ZeroHist = (TH1F*)DataRatioHist->Clone();
	//ZeroHist->Scale(0);

	//DataRatioHist->GetYaxis()->SetLabelOffset(0.1);
...
void DISControlPlotsPhotoproduction() {
	//create file to output plots to
	TFile* outputfile = new TFile(outputfilename.c_str(), "RECREATE");
	//TFile* f = new TFile(dummyfile.c_str(), "RECREATE");

	Plot(outputfile, 5, 100, 95, 0,"Q2 Distributions","Q^{2} Distributions","Q^{2} [GeV^{2}]", "1/Luminosity dN/dQ^{2} [nb GeV^{-2}]");
	cout << "Q2 done" << endl;
	Plot(outputfile, 0.1, 0.6, 100, 1, "y Distributions", "y Distributions", "y", "1/Luminosity dN/dy [nb]");
	cout << "Y done" << endl;
	Plot(outputfile, -30, 30, 100, 2, "Z Vtx Distributions", "Z_{Vtx} Distributions", "Z_{vtx} [cm]", "1/Luminosity dN/dZ_{vtx} [nb cm^{-1}]");
	cout << "Z Vtx done" << endl;
	Plot(outputfile, 35, 70, 100, 3, "Epz Distributions", "Epz Distributions", "E-p_{z} [GeV]", "1/Luminosity dN/d(E-p_{z}) [nb GeV^{-1}]");
	cout << "Epz done" << endl;
	Plot(outputfile, 11, 32, 100, 4, "Ee Distributions", "E_{e} Distributions", "E^{'}_{e} [GeV]", "1/Luminosity dN/dE^{'}_{e} [nb GeV^{-1}]");
	cout << "Ee done" << endl;
	Plot(outputfile, -3.14, 3.14, 50, 5, "Phi Distributions", "#phi Distributions", "#phi_{e} [rad]", "1/Luminosity dN/d#phi_{e} [nb rad^{-1}] ");
	cout << "Phi done" << endl;
	Plot(outputfile, 2.63, 3.08, 50, 6, "Theta Distributions", "#theta Distributions", "#theta_{e} [rad]", "1/Luminosity dN/d#theta_{e} [nb rad^{-1}]");
	cout << "Theta done" << endl;
	Plot(outputfile, 2, 3.52, 50, 7, "Ecra Distributions", "Ecra Distributions", "R_{cluster} [cm]", "1/Luminosity dN/dR_{cluster} [nb cm^{-1}]");
	cout << "Ecra done" << endl;

	Plot(outputfile, -70, 70, 140, 11, "Xcluster Distributions", "Xcluster Distributions", "X_{SpaCal} [cm]", "1/Luminosity dN/dX_{SpaCal} [nb cm^{-1}]");
	cout << "Xcluster done" << endl;
	Plot(outputfile, -70, 70, 140, 12, "Ycluster Distributions", "Ycluster Distributions", "Y_{SpaCal} [cm]", "1/Luminosity dN/dY_{SpaCal} [nb cm^{-1}]");
	cout << "Xcluster done" << endl;
	Plot(outputfile, -163, -160, 100, 13, "Zcluster Distributions", "Zcluster Distributions", "Z_{SpaCal} [cm]", "1/Luminosity dN/dZ_{SpaCal} [nb cm^{-1}]");
	cout << "Xcluster done" << endl;
	


	Plot(outputfile, 0.0, 0.1, 100, 14, "Bjorken x Distributions", "Bjorken x", "x", "1/Luminosity dN/dx [nb]");
