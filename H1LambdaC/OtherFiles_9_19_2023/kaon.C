...
main(int argc, char* argv[])
{
    ...
    TTree* K0Tree = new TTree("Lambda Reg", "Lambda Reg");
...
////////Get Raw pT and Eta distributions before implementing those cuts/////////////////////////////////////////////////////////////////////////////
        ...
        //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
                // STORE K0 CANDIDATES INTO TTREES AFTER ALL CUTS EXCEPT THE PT AND ETA CUT HERE
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
       ...
       //}
        ...
        for (Int_t loop2 = 0; loop2 < PST.GetEntries(); loop2++) {
            ...
            if (PST[loop2]->GetPt() < 0.60) { continue; }
            ...
            double pTrelC = vecLambdaC.Cross(vecProtonC).Mag() / (vecLambdaC.Mag());
            double qlProtonC = vecLambdaC.Dot(vecProtonC) / vecLambdaC.Mag();
            double qlLambdaC = vecLambdaC.Dot(K0Cand[loop]->GetFourVector().Vect()) / vecLambdaC.Mag();
            double alphaC = (2 * (qlProtonC)) / (vecLambdaC.Mag()) - 1;

            
            if ((sqrt(vecLambdaC.X() * vecLambdaC.X() + vecLambdaC.Y() * vecLambdaC.Y())) < 1.5) { continue; }
            hC5->Fill(invMasslambdaC);

            
            
            //ensuring the larger longitudinal momentum with respect to charm lambda is the lambda, skip if not
            if (qlProtonC < qlLambdaC) { continue; }
            hC6->Fill(invMasslambdaC);

            hAPbeforeC->Fill(alphaC, pTrelC);

            if (pTrelC < 0.3 || pTrelC > 0.889) { continue; } //pT,rel cut for Armenteros Plot
       ...
       gPad->Update();
       //canvas->SaveAs("kaonAPbefore.root");
       canvas->SetName("Charm AP Before");
       canvas->Write();
