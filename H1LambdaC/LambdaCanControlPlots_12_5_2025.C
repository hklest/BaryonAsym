...
string fileDataTree = "/nfs/dust/h1/group/gtustin/h1oo/H1LambdaC/LambdaTree/merged.root";
string fileRAPGAPTree = "/nfs/dust/h1/group/gtustin/h1oo/H1LambdaC_MC/LambdaTreeMC/RAPGAP31/merged.root";
string fileDJANGOHTree = "/nfs/dust/h1/group/gtustin/h1oo/H1LambdaC_MC/LambdaTreeMC/DJANGOH14/merged.root";
...
int Plot(TFile* output,double lowerbinrange, double upperbinrange, double bins, int variable, string treebranch, string canvasname, string plottitle, string xtitle, string ytitle) {
	...
	for (m = 0; m < entries; m++) {
		...
		if (pT > 0.5 && pT < 3.5 && Ys>0.0375 && Ys < 0.6 && Q2s>5 && Q2s < 100 && abs(Eta) < 1.3) {
			if (variable == 0) {
				DataEventTree->GetEntry(m);
	...
	DataHist->GetYaxis()->SetTitleOffset(1.85);
