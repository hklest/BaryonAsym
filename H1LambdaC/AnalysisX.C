...
string outputfilename = "/nfs/dust/h1/group/gtustin/h1oo/H1LambdaC/AnalysisX.root";
...
void AnalysisX() {
	...
	LambdaTotDataCount = AnalyzeTree(new TFile(filelambdaTree.c_str(), "read"), "Lambda Tot", xentries, 0, LambdaTotDatax, xbins, "Lambda Tot Data x Extraction ", c3, c1);
	LambdaTotMCCount = AnalyzeTree(new TFile(filelambdaTreeMC.c_str(), "read"), "Lambda Tot", xentries, 0, LambdaTotMCx, xbins, "Lambda Tot MC x Extraction ", c3, c1);

	LambdaRegDataCount = AnalyzeTree(new TFile(filelambdaTree.c_str(), "read"), "Lambda Reg", xentries, 0, LambdaRegDatax, xbins, "Lambda Reg Data x Extraction ", c3, c1);
	LambdaRegMCCount = AnalyzeTree(new TFile(filelambdaTreeMC.c_str(), "read"), "Lambda Reg", xentries, 0, LambdaRegMCx, xbins, "Lambda Reg MC x Extraction ", c3, c1);

	LambdaBarDataCount = AnalyzeTree(new TFile(filelambdaTree.c_str(), "read"), "Lambda Bar", xentries, 0, LambdaBarDatax, xbins, "Lambda Bar Data x Extraction ", c3, c1);
	LambdaBarMCCount = AnalyzeTree(new TFile(filelambdaTreeMC.c_str(), "read"), "Lambda Bar", xentries, 0, LambdaBarMCx, xbins, "Lambda Bar MC x Extraction ", c3, c1);

	KaonDataCount = AnalyzeTree(new TFile(filekaonTree.c_str(), "read"), "K0", xentries, 1, KaonDatax, xbins, "Kaon Tot Data x Extraction ", c3, c1);
	KaonMCCount = AnalyzeTree(new TFile(filekaonTreeMC.c_str(), "read"), "K0", xentries, 1, KaonMCx, xbins, "Kaon Tot MC x Extraction ", c3, c1);

	//Analyzing Gen Counts
	LambdaTotGenCount = AnalyzeGenTree(new TFile(filegenMCTree.c_str(), "read"), "Lambda Tot", xentries, xbins);
	LambdaRegGenCount = AnalyzeGenTree(new TFile(filegenMCTree.c_str(), "read"), "Lambda Reg", xentries, xbins);
	LambdaBarGenCount = AnalyzeGenTree(new TFile(filegenMCTree.c_str(), "read"), "Lambda Bar", xentries, xbins);
	KaonGenCount = AnalyzeGenTree(new TFile(filegenMCTree.c_str(), "read"), "K0", xentries, xbins);
	

	
	double y1[xentries] = {};
	double xerr[xentries] = { 0.00003,0.00005,0.0001,0.0003,0.0045 };
	double xvals[xentries] = { 0.00007,0.00015,0.0003,0.0007,0.0055 };
	double y1err[xentries] = {};
	//double x[xentries] = { 0.55,0.65,0.75,0.85,1.0,1.2,1.45,1.9,2.85 };
	//double dx[xentries] = { 0.1,0.1,0.1,0.1,0.2,0.2,0.3,0.6,1.3 };

	double effregplot[xentries] = {};
	double effbarplot[xentries] = {};
	double erreffregplot[xentries] = {};
	double erreffbarplot[xentries] = {};

	for (int k = 0; k < xentries; k++) {
		...
		double templamcrosssection = (LambdaTotDataCount[k][0]) / (templameff * BRlambda * DataIntLumi06 * (xbins[k+1]-xbins[k]));
		
		cout << "Lambda count: " << LambdaTotDataCount[k][0] << endl;
		cout << "eff: " << templameff << endl;
		cout << "diff: " << xbins[k + 1] - xbins[k] << endl;

		double templamcrosssectionerr = GetRatioError(templamcrosssection, (LambdaTotDataCount[k][0]), (LambdaTotDataCount[k][1]), (templameff * BRlambda * DataIntLumi06 * (xbins[k + 1] - xbins[k])), templamefferr);
	...
	double dyup[5];
	int n = 0;
	X[n] = xvals[n];	y[n] = 28100;	dxlow[n] = 0;	dxup[n] = 0;	dylow[n] = 0.55;	dyup[n] = 0.55;	n++;
	X[n] = xvals[n];	y[n] = 20100;	dxlow[n] = 0;	dxup[n] = 0;	dylow[n] = 0.35;	dyup[n] = 0.35;	n++;
	X[n] = xvals[n];	y[n] = 8500;	dxlow[n] = 0;	dxup[n] = 0;	dylow[n] = 0.15;	dyup[n] = 0.15;	n++;
	X[n] = xvals[n];	y[n] = 2570;	dxlow[n] = 0;	dxup[n] = 0;	dylow[n] = 0.04;	dyup[n] = 0.04;	n++;
	X[n] = xvals[n];	y[n] = 104;	dxlow[n] = 0;	dxup[n] = 0;	dylow[n] = 0.0015;	dyup[n] = 0.0015;	n++;
	////

	double dylow2[5];
	double dyup2[5];
	int m = 0;
	dylow2[m] = 2.0;	dyup2[m] = 2.0;	m++;
	dylow2[m] = 1.5;	dyup2[m] = 1.4;	m++;
	dylow2[m] = 0.5;	dyup2[m] = 0.5;	m++;
	dylow2[m] = 0.14;	dyup2[m] = 0.15;	m++;
	...
	auto effreg = new TGraphErrors(xentries, X, effregplot, xerr, erreffregplot);
	...
	auto cs = new TGraphErrors(xentries, X, y1, xerr, y1err);
	PlotData("Lambda Cross Section vs x", "d#sigma(ep #rightarrow e [#Lambda + #bar{#Lambda}] X)/dx", "x", "d#sigma/dx",0,4.2, cs, xentries, X, y, xerr, y1err, dylow, dyup, dylow2, dyup2);
	...
	n = 0;
	X[n] = xvals[n];	y[n] = 0.00955056;	dxlow[n] = 0;	dxup[n] = 0;	dylow[n] = 0.0404494;	dyup[n] = 0.0404494;	n++;
	X[n] = xvals[n];	y[n] = -0.000561798;	dxlow[n] = 0;	dxup[n] = 0;	dylow[n] = 0.0320225;	dyup[n] = 0.0320225;	n++;
	X[n] = xvals[n];	y[n] = -0.0410112;	dxlow[n] = 0;	dxup[n] = 0;	dylow[n] = 0.0303371;	dyup[n] = 0.0303371;	n++;
	X[n] = xvals[n];	y[n] = 0.0011236;	dxlow[n] = 0;	dxup[n] = 0;	dylow[n] = 0.0370787;	dyup[n] = 0.0370787;	n++;
	X[n] = xvals[n];	y[n] = 0.0735955;	dxlow[n] = 0;	dxup[n] = 0;	dylow[n] = 0.0303371;	dyup[n] = 0.0303371;	n++;
	///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	auto cs2 = new TGraphErrors(xentries, X, y1, xerr, y1err);
	PlotData("Lambda Asymm vs x", "[#sigma(ep #rightarrow e #Lambda X) - #sigma(ep #rightarrow e #bar{#Lambda} X)] / [#sigma(ep #rightarrow e #Lambda X) + #sigma(ep #rightarrow e #bar{#Lambda} X)]", "x", "A_{#Lambda}",-0.15,0.15, cs2, xentries, X, y, xerr, y1err, dylow, dyup, dylow, dyup);
	
	//K0 Cross section results from 2009 H1 Paper
	n = 0;
	X[n] = xvals[n];	y[n] = 69.4;	dxlow[n] = 0;	dxup[n] = 0;	dylow[n] = 0.5;	dyup[n] = 0.5;	n++;
	...
	for (int k = 0; k < xentries; k++) {
		...
		double tempk0crosssection = (KaonDataCount[k][0]) / (tempk0eff * BRkaon * DataIntLumi06 * (xbins[k + 1] - xbins[k]));

		double tempk0crosssectionerr = GetRatioError(tempk0crosssection, (KaonDataCount[k][0]), (KaonDataCount[k][1]), (tempk0eff * BRkaon * DataIntLumi06 * (xbins[k + 1] - xbins[k])), tempk0efferr);
	...
	auto effreg2 = new TGraphErrors(xentries, X, effregplot, xerr, erreffregplot);
	//auto effbar = new TGraphErrors(arraysize, X, effbarplot, xerr, erreffbarplot);
	...
	auto cs3 = new TGraphErrors(xentries, X, y1, xerr, y1err);
	PlotData("K0 Cross Section vs x", "d#sigma(ep #rightarrow e K^{0}_{S} X)/dx", "x", "d#sigma/dx [nb]",0,10.5, cs3, xentries, X, y, xerr, y1err, dylow, dyup, dylow2, dyup2);
	...
	n = 0;
	X[n] = xvals[n];	y[n] = 0.405;	dxlow[n] = 0;	dxup[n] = 0;	dylow[n] = 0.0085;	dyup[n] = 0.0085;	n++;
	...
	auto cs4 = new TGraphErrors(xentries, X, y1, xerr, y1err);
	PlotData("Lambda/K0 Cross Section vs x", "[d#sigma(ep #rightarrow e #Lambda X)]/[d#sigma(ep #rightarrow e K^{0}_{S} X)]", "x", "#Lambda/K^{0}_{S}",0,0.51, cs4, xentries, X, y, xerr, y1err, dylow, dyup, dylow2, dyup2);
	...
	//Analyzing Tot Lambda Counts for x bins for MC and Data
	LambdaTotDataCountincl = AnalyzeTree(new TFile(filelambdaTree.c_str(), "read"), "Lambda Tot", 1, 0, LambdaTotDataxincl, {xbins[0],xbins[xentries]}, "Lambda Tot Incl Data x Extraction ", c3, c1);
	LambdaTotMCCountincl = AnalyzeTree(new TFile(filelambdaTreeMC.c_str(), "read"), "Lambda Tot", 1, 0, LambdaTotMCxincl, { xbins[0],xbins[xentries] }, "Lambda Tot Incl MC x Extraction ", c3, c1);

	LambdaRegDataCountincl = AnalyzeTree(new TFile(filelambdaTree.c_str(), "read"), "Lambda Reg", 1, 0, LambdaRegDataxincl, { xbins[0],xbins[xentries] }, "Lambda Reg Incl Data x Extraction ", c3, c1);
	LambdaRegMCCountincl = AnalyzeTree(new TFile(filelambdaTreeMC.c_str(), "read"), "Lambda Reg", 1, 0, LambdaRegMCxincl, { xbins[0],xbins[xentries] }, "Lambda Reg Incl MC x Extraction ", c3, c1);

	LambdaBarDataCountincl = AnalyzeTree(new TFile(filelambdaTree.c_str(), "read"), "Lambda Bar", 1, 0, LambdaBarDataxincl, { xbins[0],xbins[xentries] }, "Lambda Bar Incl Data x Extraction ", c3, c1);
	LambdaBarMCCountincl = AnalyzeTree(new TFile(filelambdaTreeMC.c_str(), "read"), "Lambda Bar", 1, 0, LambdaBarMCxincl, { xbins[0],xbins[xentries] }, "Lambda Bar Incl MC x Extraction ", c3, c1);

	KaonDataCountincl = AnalyzeTree(new TFile(filekaonTree.c_str(), "read"), "K0", 1, 1, KaonDataxincl, { xbins[0],xbins[xentries] }, "Kaon Incl Data x Extraction ", c3, c1);
	KaonMCCountincl = AnalyzeTree(new TFile(filekaonTreeMC.c_str(), "read"), "K0", 1, 1, KaonMCxincl, { xbins[0],xbins[xentries] }, "Kaon Incl MC x Extraction ", c3, c1);

	//Analyzing Gen Counts
	LambdaTotGenCountincl = AnalyzeGenTree(new TFile(filegenMCTree.c_str(), "read"), "Lambda Tot", 1, { xbins[0],xbins[xentries] });
	LambdaRegGenCountincl = AnalyzeGenTree(new TFile(filegenMCTree.c_str(), "read"), "Lambda Reg", 1, { xbins[0],xbins[xentries] });
	LambdaBarGenCountincl = AnalyzeGenTree(new TFile(filegenMCTree.c_str(), "read"), "Lambda Bar", 1, { xbins[0],xbins[xentries] });
	KaonGenCountincl = AnalyzeGenTree(new TFile(filegenMCTree.c_str(), "read"), "K0", 1, { xbins[0],xbins[xentries] });
