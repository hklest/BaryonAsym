...
vector<double> mlpfunction(TH1F* inHist, string plotname, int particlecode, TCanvas* c3, TCanvas* c1, vector<double> fitparaminit, vector<double> rangedefs) { //when calling function, particle code deictates 0=lambda or 1=kaon MLP parameters
	...
	double BGVALINT = ig->getVal() * n.getVal();
...
vector<double> mlpfunctionSAVE(TH1F* inHist, string plotname, int particlecode, TCanvas* c3, TCanvas* c1, vector<double> fitparaminit, vector<double> rangedefs) { //when calling function, particle code deictates 0=lambda or 1=kaon MLP parameters
	...
	// Signal+Bg PDF
	////////////////////////////////////////////////////////////////////////////////////
	RooRealVar nsig("nsig", "# signal events", 1.0, 1.0, 1000000000000);
	RooRealVar nbkg("nbkg", "# background events", 1.0, 1.0, 100000000000);
	...
	for (int i = 1; i < fgHist->GetNbinsX() + 1; i++) {
		...
		if ((inputs[0] >= s1 && inputs[0] < s2)) {
			...
			//RooProduct tempsumyield("tempsumyield", "tempsumyield", RooArgSet(*sumig, nsig+nbkg));
			//RooProduct tempbgyield("tempbgyield", "tempbgyield", RooArgSet(*bggig, nbkg));
			
			//tempsignalyieldvalue = tempsignalyield.getVal();
			//tempsignalyieldvalueerror = tempsignalyield.getPropagatedError(*result, mLam);
			//tempsumyieldvalue = tempsumyield.getVal();
			//tempbgyieldvalue = tempbgyield.getVal();
			//tempsignalyieldvalue = sigig->getVal() * (nsig.getVal() + nbkg.getVal());
			//tempsignalyieldvalue = sigig->getVal() * (ntot);
			//tempsignalyieldvalueerror = tempsignalyield.getPropagatedError(*result, mLam);
			//tempsumyieldvalue = sumig->getVal() * (nsig.getVal()+nbkg.getVal());
			...
			runningsignalyield = runningsignalyield + tempsumyieldvalue;
	...
	cout << "Particle Count 1: " << SIGNALINT << endl;
	cout << "Particle Count 2: " << totalbgcount << endl;
