...
vector<double> mlpfunction(TH1F* inHist, string plotname, int particlecode, TCanvas* c3, TCanvas* c1) { //when calling function, particle code deictates 0=lambda or 1=kaon MLP parameters
	...
	TF1* f1 = new TF1("f1", flineK0, x1, x4, 2);
	...
	cout << "Slope: " << f1->GetParameter(1) << endl;
	cout << "Slope: " << f1->GetParError(1) << endl;
	//cout << "Y Intercept: " << f1->GetParameter(0) << endl;
	//cout << "Quad Term: " << f1->GetParameter(2) << endl;
	...
	for (int i = 1; i < fgHist->GetNbinsX() + 1; i++) {
		...
		if (inputs[0] >= leftsubbound[particlecode] && inputs[0] < rightsubbound[particlecode]) {
			...
			double temp_content = fgHist->GetBinContent(i) - (f1->GetParameter(0) + (f1->GetParameter(1)) * (inputs[0]) + (f1->GetParameter(2)) * (f1->GetParameter(2)) * (inputs[0]));
			...
			fitHistsubrange->SetBinContent(i, (f1->GetParameter(0) + (f1->GetParameter(1)) * (inputs[0]) + (f1->GetParameter(2)) * (f1->GetParameter(2)) * (inputs[0])));
			...
			//if (temp_content > 0) {
				...
				bgcount = bgcount + (f1->GetParameter(0) + (f1->GetParameter(1)) * (inputs[0]) + (f1->GetParameter(2)) * (f1->GetParameter(2)) * (inputs[0]));
			//}
...
void Analysis2(int varidx, vector<double> q2bins, double DataIntLumi06, double MCDjangoh14radgen, double MCDjangoh14nonradgen, string filekaonTree, string filelambdaTree, string filekaonTreeMC, string filelambdaTreeMC, string filegenMCTree, string fileNonRadgenMCTree, string fileRadgenMCTree) {
	...
	PlotData("Lambda Asymm vs q2", "(N_#Lambda - N_#bar{#Lambda})/(N_#Lambda + N_#bar{#Lambda}) in Gen Level MC", "q2", "A_{#Lambda}", -0.15, 0.15, cs2, q2entries, xvals, y, xerr, y1err, dylow, dyup, dylow, dyup);
