...
main(int argc, char* argv[])
{
    ...
    //Documented mass of lambda baryon
    double mLambda = 1.115683;
    ...
    while (H1Tree::Instance()->Next() && !opts.IsMaxEvent(eventCounter)) {
        ...
        vector<double> testvec = {1.2,1.1,1.2};
        
        vector<vector<int>> testfind = findduplicates(testvec);
        
        if (findduplicates(testvec).size()==0) {
            cout << "There are no duplicate entries" << endl;
        }

        if (findduplicates(testvec).size() != 0) {
            cout << "There are " << testfind.size() << " duplicate entries." << endl;
            for (Int_t a = 0; a < testfind.size(); a++) {
                cout << "Repeat value " << a + 1 << " occurrs at the following indices: " << endl;
                for (Int_t b = 0; b < testfind[a].size(); b++) {
                    cout << "Index Place: " << testfind[a][b] << endl;
                }
            }
        }*/

        //Tests for my minfinder
        //vector<double> testvec = {0.42};
        //int testindex = minfinder(testvec);
        //cout << "Index Min is " << testindex << endl;
...

//Now before filling Trees, I want to check for multiple V0 particle candidates coming from the same proton and pion tracks which would cause over counting particles
//This is done by relooping through the H1PartLambda class and comparing track information for redundancies
        //for (Int_t loop2 = 0; loop2 < LambdaCand.GetEntries(); loop2++) {
        //    //don't want to compare particle to itself, so skip entry where loop == loop2
        //    if (loop == loop2) { continue; } 
        //    cout << LambdaCand[loop2]->GetProton()->GetPt() << endl;
        //    if (LambdaCand[loop]->GetProton()->GetPt() == LambdaCand[loop2]->GetProton()->GetPt()) {
        //        cout << "Same proton track" << endl;
        //    }
        //    if (LambdaCand[loop]->GetPion()->GetPt() == LambdaCand[loop2]->GetPion()->GetPt()) {
        //        cout << "Same pion track" << endl;
        //    }
        //}

        //Lambdas passed all cuts so far excpet for pT and Eta ranges, saved index of candidate in vector
        LambdaIndices.push_back(loop);
        ProtonTrackCheck.push_back(LambdaCand[loop]->GetProton()->GetPt());
        PionTrackCheck.push_back(LambdaCand[loop]->GetPion()->GetPt());
        LambdaInvMasses.push_back(invMasslambda);
        PIDProtonTracks.push_back(ProtonPSTrack->GetDedxLikelihood(H1Dedx::kProton));
        PIDPionTracks.push_back(PionPSTrack->GetDedxLikelihood(H1Dedx::kPion));
        passedinitialcutscount++;
        //cout << LambdaCand[loop]->GetProton()->GetPt() << endl;
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        //vector<double> UsedProtonTracks = {};
        //vector<double> UsedPionTracks = {};
        vector<int> RejectedLambdaIndices = {};
        vector<vector<double>> deltaInvMassesproton = {};
        vector<double> MinDeltaInvMassesproton = {};
        vector<vector<double>> deltaInvMassespion = {};
        vector<double> MinDeltaInvMassespion = {};
        vector<int> NewLambdaIndices = {};
        int rejectedindexcountproton = 0;
        int rejectedindexcountpion = 0;
        int doublerejectcount = 0;
        
        vector<vector<int>> protontrackcheck = findduplicates(ProtonTrackCheck);
        vector<vector<int>> piontrackcheck = findduplicates(PionTrackCheck);
        //}
        
        //LambdaIndices.push_back(loop);
        //ProtonTrackCheck.push_back(LambdaCand[loop]->GetProton()->GetPt());
        //PionTrackCheck.push_back(LambdaCand[loop]->GetPion()->GetPt());
        //LambdaInvMass.push_back(invMasslambda);
        //PIDProtonTracks.push_back(ProtonPSTrack->GetDedxLikelihood(H1Dedx::kProton));
        //PIDPionTracks.push_back(PionPSTrack->GetDedxLikelihood(H1Dedx::kPion));
        //passedallcutscount++;
        

        //If no Lambda candidates existed from the event, then just continue to next event
        if (LambdaIndices.size() == 0) {
            //cout << "There was no lambda candidate, skipping event." << endl;
            continue; 
        }

        //If there is only one lambda candidate that passed our cuts, then there is no need to modify lambda Indices, just reassign
        //if (LambdaIndices.size() == 1) {
            //cout << "There was only one lambda candidate, passing them through." << endl;
            //NewLambdaIndices = LambdaIndices;
        //}

        //If there are multiple lambda candidates, AND there are no duplicate proton OR pion trakcs, then there is no need to modify lambda Indices, just reassign
        //if (LambdaIndices.size() > 1 && protontrackcheck.size()==0 && piontrackcheck.size()==0) {
        //    cout << "There were multiple candidates, but no redundant track information, pass them through." << endl;
            //NewLambdaIndices = LambdaIndices;
        //}

        //Analyze the case where there are duplicate proton tracks
        if (LambdaIndices.size() > 1 && protontrackcheck.size() != 0) {
            //cout << "There were multiple candidates and redundant proton tracks, analyzing:" << endl;
            for (Int_t i = 0; i < protontrackcheck.size(); i++) {
                deltaInvMassesproton.push_back({});
                //cout << "Proton Duplicate " << i + 1 << " of " << protontrackcheck.size() << ":" << endl;
                for (Int_t j = 0; j < protontrackcheck[i].size(); j++) {
                    //cout << "pT value " << j + 1 << " of " << protontrackcheck[i].size() << ": " << ProtonTrackCheck[protontrackcheck[i][j]] << endl;
                    //cout << "InvMass: " << LambdaInvMasses[protontrackcheck[i][j]] << endl;
                    //cout << "Original Lambda Candidate Index: " << LambdaIndices[protontrackcheck[i][j]] << endl;
                    //cout << "Delta InvMass: " << abs(LambdaInvMasses[protontrackcheck[i][j]] - mLambda) << endl;
                    deltaInvMassesproton[i].push_back(abs(LambdaInvMasses[protontrackcheck[i][j]] - mLambda));
                }
            }

            //cout << "Consistnecy Check:" << endl;

            //Now analyzing vectors of duplicate proton trakcs to find the ones with the minimum delta inv masses
            for (Int_t i = 0; i < protontrackcheck.size(); i++) {
                MinDeltaInvMassesproton.push_back(minfinder(deltaInvMassesproton[i]));
            }

            for (Int_t i = 0; i < protontrackcheck.size(); i++) {
                //cout << "Index of Good Proton Duplicate " << i + 1 << " of " << protontrackcheck.size() << ":" << endl;
                //cout << LambdaIndices[protontrackcheck[i][MinDeltaInvMassesproton[i]]] << endl;
                
                for (Int_t j = 0; j < protontrackcheck[i].size(); j++) {
                    if (LambdaIndices[protontrackcheck[i][j]] != LambdaIndices[protontrackcheck[i][MinDeltaInvMassesproton[i]]]) {
                        //cout << "Rejecting Original Lambda Index: " << LambdaIndices[protontrackcheck[i][j]] << endl;
                        RejectedLambdaIndices.push_back(LambdaIndices[protontrackcheck[i][j]]);
                        rejectedindexcountproton++;
                    }
        ...
        }//End of if loop that adds indices to rejected indices for proton track duplicates

        

        if (LambdaIndices.size() > 1 && piontrackcheck.size() != 0) {
            //cout << "There were multiple candidates and redundant pion tracks, analyzing:" << endl;
            for (Int_t i = 0; i < piontrackcheck.size(); i++) {
                deltaInvMassespion.push_back({});
                //cout << "Pion Duplicate " << i + 1 << " of " << piontrackcheck.size() << ":" << endl;
                for (Int_t j = 0; j < piontrackcheck[i].size(); j++) {
                    //cout << "pT value " << j + 1 << " of " << piontrackcheck[i].size() << ": " << PionTrackCheck[piontrackcheck[i][j]] << endl;
                    //cout << "InvMass: " << LambdaInvMasses[piontrackcheck[i][j]] << endl;
                    //cout << "Original Lambda Candidate Index: " << LambdaIndices[piontrackcheck[i][j]] << endl;
                    //cout << "Delta InvMass: " << abs(LambdaInvMasses[piontrackcheck[i][j]] - mLambda) << endl;

                    deltaInvMassespion[i].push_back(abs(LambdaInvMasses[piontrackcheck[i][j]] - mLambda));
                }
            }

            //cout << "Consistnecy Check:" << endl;

            //Now analyzing vectors of duplicate pion trakcs to find the ones with the minimum delta inv masses
            for (Int_t i = 0; i < piontrackcheck.size(); i++) {
                MinDeltaInvMassespion.push_back(minfinder(deltaInvMassespion[i]));
            }

            for (Int_t i = 0; i < piontrackcheck.size(); i++) {
                //cout << "Index of Good Pion Duplicate " << i + 1 << " of " << piontrackcheck.size() << ":" << endl;
                //cout << LambdaIndices[piontrackcheck[i][MinDeltaInvMassespion[i]]] << endl;

                for (Int_t j = 0; j < piontrackcheck[i].size(); j++) {
                    
                    if (LambdaIndices[piontrackcheck[i][j]] != LambdaIndices[piontrackcheck[i][MinDeltaInvMassespion[i]]]) {
                        //cout << "Rejecting Original Lambda Index: " << LambdaIndices[piontrackcheck[i][j]] << endl;
                        if (intsinglefind(RejectedLambdaIndices, 0, RejectedLambdaIndices.size(), LambdaIndices[piontrackcheck[i][j]]) != -1) { doublerejectcount++; }
                        RejectedLambdaIndices.push_back(LambdaIndices[piontrackcheck[i][j]]);
                        rejectedindexcountpion++;
                    }
        ...
        }//End of if loop that adds indices to rejected indices for pion track duplicates
        


        

        ////Loop over LambdaCandidates
        //for (Int_t a = 0; a < LambdaIndices.size(); a++) {
        //    //If index I am using now exists in the Rejected Indices array, I will skip it
        //    //int intsinglefind(vector<int> vec, int begin_index, int end_index, double value)
        //    if (intsinglefind(RejectedLambdaIndices,0, RejectedLambdaIndices.size(), LambdaIndices[a])!=-1) { continue; }

        //    // Redefine variables, brach reminder
        //    //lambdaRegTree->Branch("pT", &pT, "pT/D");
        //    //lambdaRegTree->Branch("Angle", &Angle, "Angle/D");
        //    //lambdaRegTree->Branch("v0chi2", &v0chi2, "v0chi2/D");
        //    
        //    Eta = LambdaCand[LambdaIndices[a]]->GetFourVector().Vect().Eta();
        //    invMasslambda = (LambdaCand[LambdaIndices[a]]->GetProtonFourVector()
        //        + LambdaCand[LambdaIndices[a]]->GetPionFourVector()).M();
        //    
        //    // Defining TTree variables to store into branches
        //    Eta = LambdaCand[loop]->GetFourVector().Vect().Eta();
        //    //x = (*q2e) / ((*ye) * (*ebeamE) * (*ebeamP) * 4);
        ...
        //    Q2s = *q2s;
        //    Ys = *ys;
        //    pT = LambdaCand[LambdaIndices[a]]->GetV0()->GetPt();
        //    v0chi2 = LambdaCand[LambdaIndices[a]]->GetV0()->GetFitChi2();
        ...
        //    if (LambdaCand[LambdaIndices[a]]->IsLambdaBar() == true) {
        //        lambdaBarTree->Fill();
        //        if (pT > 0.5 && pT < 3.5) {
        //            if (abs(Eta) < 1.3) {
        //                if (Ys > 0.1 && Ys < 0.6) {
        ...
        //    }
        //
        //}//End of second loop over lambda candidates

        //if (passedinitialcutscount - RejectedLambdaIndices.size() + doublerejectcount != passedallcutscount) { cout << "Bad Count" << endl; }
