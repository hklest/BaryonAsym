...
//MLP function, this function does the mlp fit and peak extraction, output is a 3 component vecotr {total particle peak count after BG subtraction, error in the peak count, total BG count}
vector<double> roofitBG(TH1F* inHist, string plotname, int particlecode, TCanvas* c3, TCanvas* c1) { //when calling function, particle code deictates 0=lambda or 1=kaon MLP parameters
	...
	RooRealVar mu("mu", "mu", 1.1156, 1.11, 1.12);
...
vector<double> mlpfunction(TH1F* inHist, string plotname, int particlecode, TCanvas* c3, TCanvas* c1) { //when calling function, particle code deictates 0=lambda or 1=kaon MLP parameters
	...
	RooRealVar a1("a1", "a1", 0.5, 0., 5.);
	RooRealVar a2("a2", "a2", 0.0, -100., 100.);
	RooGenericPdf bg("bg", "TMath::Power(TMath::Max(mLam-01.0782,0.),a1)*TMath::Exp(a2*(mLam-1.0782))", RooArgSet(mLam, a1, a2));
	...
	RooRealVar sigma("sigma", "sigma", 0.0015);
	RooRealVar p3("p3", "p3", 5.0);
	...
	RooRealVar nbkg("nbkg", "# background events", 10., 0., 10000.);
	RooAddPdf sum("sum", "signal+bg", RooArgList(sigf, bg), RooArgList(nsig, nbkg));
	/////////////////////////////////////////////////////////////////////////////////////
	...
	RooPlot* RooFitPlot = mLam.frame();
	RooFitDataHist.plotOn(RooFitPlot);
	//bg.plotOn(RooFitPlot, RooFit::Range("full"));
	RooFitPlot->Write();
	...
	std::unique_ptr<RooAbsReal> ig{ sum.createIntegral(mLam, RooFit::Range("signal")) };
	double ntot = sum.expectedEvents(RooArgSet(mLam));  //this should be nsig + nbkg
	cout << ig->getVal() * ntot << endl;
	//std::unique_ptr<RooAbsReal> bgundersig{ sum.createIntegral(mLam, RooFit::Range("signal")) };
	double BGVALINT = ig->getVal() * ntot;
	//cout << "INT: " << BGVAL << endl;
...
void Asym(int varidx, vector<double> q2bins, double DataIntLumi06, double MCDjangoh14radgen, double MCDjangoh14nonradgen, string filekaonTree, string filelambdaTree, string filekaonTreeMC, string filelambdaTreeMCRAPGAP, string filelambdaTreeMCDJANGOH, string filegenMCTreeRAPGAP, string filegenMCTreeDJANGOH, string fileNonRadgenMCTree, string fileRadgenMCTree, string xaxistitle) {
	...
	LambdaTotMCRAPGAPCount = AnalyzeTree(varidx, new TFile(filelambdaTreeMCRAPGAP.c_str(), "read"), "Lambda Tot", q2entries, 0, LambdaTotMCRAPGAPQ2, q2bins, "Lambda Tot MC RAPGAP q2 Extraction ", c3, c1);
	LambdaTotMCDJANGOHCount = AnalyzeTree(varidx, new TFile(filelambdaTreeMCDJANGOH.c_str(), "read"), "Lambda Tot", q2entries, 0, LambdaTotMCDJANGOHQ2, q2bins, "Lambda Tot MC DJANGOH q2 Extraction ", c3, c1);
	...
	LambdaRegMCRAPGAPCount = AnalyzeTree(varidx, new TFile(filelambdaTreeMCRAPGAP.c_str(), "read"), "Lambda Reg", q2entries, 0, LambdaRegMCRAPGAPQ2, q2bins, "Lambda Reg MC RAPGAP q2 Extraction ", c3, c1);
	LambdaRegMCDJANGOHCount = AnalyzeTree(varidx, new TFile(filelambdaTreeMCDJANGOH.c_str(), "read"), "Lambda Reg", q2entries, 0, LambdaRegMCDJANGOHQ2, q2bins, "Lambda Reg MC DJANGOH q2 Extraction ", c3, c1);
	...
	LambdaBarMCRAPGAPCount = AnalyzeTree(varidx, new TFile(filelambdaTreeMCRAPGAP.c_str(), "read"), "Lambda Bar", q2entries, 0, LambdaBarMCRAPGAPQ2, q2bins, "Lambda Bar MC RAPGAP q2 Extraction ", c3, c1);
	LambdaBarMCDJANGOHCount = AnalyzeTree(varidx, new TFile(filelambdaTreeMCDJANGOH.c_str(), "read"), "Lambda Bar", q2entries, 0, LambdaBarMCDJANGOHQ2, q2bins, "Lambda Bar MC DJANGOH q2 Extraction ", c3, c1);
	...
	LambdaTotMCRAPGAPCountincl = AnalyzeTree(varidx, new TFile(filelambdaTreeMCRAPGAP.c_str(), "read"), "Lambda Tot", 1, 0, LambdaTotMCRAPGAPxincl, { q2bins[0],q2bins[q2entries] }, "Lambda Tot Incl MC RAPGAP q2 Extraction ", c3, c1);
	LambdaTotMCDJANGOHCountincl = AnalyzeTree(varidx, new TFile(filelambdaTreeMCDJANGOH.c_str(), "read"), "Lambda Tot", 1, 0, LambdaTotMCDJANGOHxincl, { q2bins[0],q2bins[q2entries] }, "Lambda Tot Incl MC DJANGOH q2 Extraction ", c3, c1);

	
	LambdaTotMCRAPGAPgenassignedCountincl = AnalyzeTree(varidx, new TFile(filelambdaTreeMCRAPGAP.c_str(), "read"), "Lambda Reg Gen", 1, 0, LambdaTotMCRAPGAPgenassigned, { q2bins[0],q2bins[q2entries] }, "Lambda Reg Gen Incl MC RAPGAP q2 Extraction ", c3, c1);
	LambdaTotMCDJANGOHgenassignedCountincl = AnalyzeTree(varidx, new TFile(filelambdaTreeMCDJANGOH.c_str(), "read"), "Lambda Reg Gen", 1, 0, LambdaTotMCDJANGOHgenassigned, { q2bins[0],q2bins[q2entries] }, "Lambda Reg Gen Incl MC DJANGOH q2 Extraction ", c3, c1);
	
	LambdaTotMCRAPGAPgenassignedBGCountincl = AnalyzeTree(varidx, new TFile(filelambdaTreeMCRAPGAP.c_str(), "read"), "Lambda Reg BG", 1, 0, LambdaTotMCRAPGAPgenassignedBG, { q2bins[0],q2bins[q2entries] }, "Lambda Reg BG Incl MC RAPGAP q2 Extraction ", c3, c1);
	LambdaTotMCDJANGOHgenassignedBGCountincl = AnalyzeTree(varidx, new TFile(filelambdaTreeMCDJANGOH.c_str(), "read"), "Lambda Reg BG", 1, 0, LambdaTotMCDJANGOHgenassignedBG, { q2bins[0],q2bins[q2entries] }, "Lambda Reg BG Incl MC DJANGOH q2 Extraction ", c3, c1);
	...
	LambdaRegMCRAPGAPCountincl = AnalyzeTree(varidx, new TFile(filelambdaTreeMCRAPGAP.c_str(), "read"), "Lambda Reg", 1, 0, LambdaRegMCRAPGAPxincl, { q2bins[0],q2bins[q2entries] }, "Lambda Reg Incl MC RAPGAP q2 Extraction ", c3, c1);
	LambdaRegMCDJANGOHCountincl = AnalyzeTree(varidx, new TFile(filelambdaTreeMCDJANGOH.c_str(), "read"), "Lambda Reg", 1, 0, LambdaRegMCDJANGOHxincl, { q2bins[0],q2bins[q2entries] }, "Lambda Reg Incl MC DJANGOH q2 Extraction ", c3, c1);
	...
	LambdaBarMCRAPGAPCountincl = AnalyzeTree(varidx, new TFile(filelambdaTreeMCRAPGAP.c_str(), "read"), "Lambda Bar", 1, 0, LambdaBarMCRAPGAPxincl, { q2bins[0],q2bins[q2entries] }, "Lambda Bar Incl MC RAPGAP q2 Extraction ", c3, c1);
	LambdaBarMCDJANGOHCountincl = AnalyzeTree(varidx, new TFile(filelambdaTreeMCDJANGOH.c_str(), "read"), "Lambda Bar", 1, 0, LambdaBarMCDJANGOHxincl, { q2bins[0],q2bins[q2entries] }, "Lambda Bar Incl MC DJANGOH q2 Extraction ", c3, c1);
