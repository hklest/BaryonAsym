...
vector<double> mlpfunction(TH1F* inHist, string plotname, int particlecode, TCanvas* c3, vector<double> fitparaminit, vector<double> rangedefs) { //when calling function, particle code deictates 0=lambda or 1=kaon MLP parameters
	...
	RooRealVar sigma("sigma", "sigma", fitparaminit[0], 0.0008, 0.006);
	...
	sum.fitTo(RooFitDataHist, RooFit::Range("roughSIG"));
...
void AsymIntegrated(int varidx, vector<double> q2bins, double DataIntLumi06, double MCDjangoh14radgen, double MCDjangoh14nonradgen, string filekaonTree, string filelambdaTree, string filekaonTreeMC, string filelambdaTreeMCRAPGAP, string filelambdaTreeMCDJANGOH, string filegenMCTreeRAPGAP, string filegenMCTreeDJANGOH, string fileNonRadgenMCTree, string fileRadgenMCTree, string xaxistitle, vector<vector<double>> binparaminitsLambda, vector<vector<double>> binrangedefsLambda, vector<vector<double>> binparaminitsLambdaBar, vector<vector<double>> binrangedefsLambdaBar, vector<vector<double>> binparaminitsMCLambda, vector<vector<double>> binrangedefsMCLambda, vector<vector<double>> binparaminitsMCLambdaBar, vector<vector<double>> binrangedefsMCLambdaBar) {
	...
	*/
	...
	LambdaRegDataCountincl = AnalyzeTree(varidx, new TFile(filelambdaTree.c_str(), "read"), "Lambda Reg", 1, 0, LambdaRegDataxincl, { q2bins[0],q2bins[q2entries] }, "#Lambda Data Extraction", c3, c1, { {0.00205,3.0,1.1156} }, {{ 1.0782,1.095,1.135,1.155 } });
	LambdaRegMCRAPGAPCountincl = AnalyzeTree(varidx, new TFile(filelambdaTreeMCRAPGAP.c_str(), "read"), "Lambda Reg", 1, 0, LambdaRegMCRAPGAPxincl, { q2bins[0],q2bins[q2entries] }, "#Lambda RAPGAP Extraction", c3, c1, { {0.0017, 1.5,1.1156} }, { {1.0782,1.095,1.135,1.155} });
	LambdaRegMCDJANGOHCountincl = AnalyzeTree(varidx, new TFile(filelambdaTreeMCDJANGOH.c_str(), "read"), "Lambda Reg", 1, 0, LambdaRegMCDJANGOHxincl, { q2bins[0],q2bins[q2entries] }, "#Lambda DJANGOH Extraction", c3, c1, { {0.0017, 1.5,1.1156} }, { {1.0782,1.095,1.135,1.155} });

	LambdaBarDataCountincl = AnalyzeTree(varidx, new TFile(filelambdaTree.c_str(), "read"), "Lambda Bar", 1, 0, LambdaBarDataxincl, { q2bins[0],q2bins[q2entries] }, "#bar{#Lambda} Data Extraction", c3, c1, { {0.00209,3.0,1.1156} }, {{ 1.0782,1.095,1.135,1.155 } });
	LambdaBarMCRAPGAPCountincl = AnalyzeTree(varidx, new TFile(filelambdaTreeMCRAPGAP.c_str(), "read"), "Lambda Bar", 1, 0, LambdaBarMCRAPGAPxincl, { q2bins[0],q2bins[q2entries] }, "#bar{#Lambda} RAPGAP Extraction", c3, c1, { {0.002,4.0,1.1156} }, {{ 1.0782,1.095,1.138,1.155 } });
	LambdaBarMCDJANGOHCountincl = AnalyzeTree(varidx, new TFile(filelambdaTreeMCDJANGOH.c_str(), "read"), "Lambda Bar", 1, 0, LambdaBarMCDJANGOHxincl, { q2bins[0],q2bins[q2entries] }, "#bar{#Lambda} DJANGOH Extraction", c3, c1, { {0.002,4.0,1.1156} }, {{ 1.0782,1.095,1.138,1.155 } });
	...
	double INCAsymmerrSYS = sqrt(Square(INCMCerrorDirectDiff) + Square(INCextractionrangeDirectDiff) + Square(INCrawextractionDirectDiff) + Square(INCalternativeBGfitDirectDiff));
