...
precheck_submit_arguments() {
  local file="$1"
  python3 - <<'PY2' "$file" "$MINIMAL" "$DRY_RUN" "${AUTO_GENERATE_MINIMAL_STEERING:-1}"
import re, shlex, sys
from pathlib import Path
...
if steer:
    sp = Path(steer)
    if not sp.exists() and auto_gen:
        txt = None
        s = str(sp)
        m = re.search(r'SelectedEvents06_(\d+)\.steer$', s)
        if m:
            idx = int(m.group(1))
            txt = f"""H1SteerTree() {{
   // auto-generated by pipeline precheck
   fHatFiles  = \"/pnfs/desy.de/dphep/online/h1/data/oo-4.0/06/hat.4.0.6.10dst7.c06{idx:04d}.root\";
   fModsFiles = \"/pnfs/desy.de/dphep/online/h1/data/oo-4.0/06/mods.4.0.6.10dst7.c06{idx:04d}.root\";
}}
"""

        m = re.search(r'RAPGAP31_06RAD_(\d+)\.steer$', s)
...
"""
        ...
        m = re.search(r'Tree06_(\d+)\.steer$', s)
        if m and txt is None:
            idx = int(m.group(1))
            txt = f"""H1SteerTree() {{
  fEventListFiles = \"/data/dust/group/h1/klesthen/BaryonAsym/H1LambdaC/EventLists/2006/condor_06_evls_{idx}.root\";
}}
"""

        m = re.search(r'TreeMC_RAPGAP31_06_RAD_(\d+)\.steer$', s)
        if m and txt is None:
            idx = int(m.group(1))
            txt = f"""H1SteerTree() {{
  fEventListFiles = \"/data/dust/group/h1/klesthen/BaryonAsym/H1LambdaC_MC/EventLists/RAPGAP31/2006/condor_06_evlsMC_RAPGAP31_Rad_{idx}.root\";
}}
"""

        if txt is not None:
            if dry_run:
                msgs.append(f'WARN::Would auto-generate missing steering file: {sp}')
            else:
                sp.parent.mkdir(parents=True, exist_ok=True)
                sp.write_text(txt)
                msgs.append(f'INFO::Auto-generated steering file: {sp}')
...
generate_tree_steerings_from_eventlists() {
  local generator="$ROOT_DIR/analysis_unified/generate_tree_steering_from_eventlists.sh"
  if [[ ! -x "$generator" ]]; then
    warn "Steering generator not found: $generator"
    return 0
  fi
  if [[ "$DRY_RUN" == "1" ]]; then
    log "DRY-RUN: $generator"
    return 0
  fi
  log "Generating Tree steering files from produced event lists"
  "$generator" || warn "Steering generation script reported issues"
}

wait_and_reset_clusters() {
  if [[ "$DO_WAIT" == "1" && "$WAIT_BETWEEN_STAGES" == "1" ]]; then
