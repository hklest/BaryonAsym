...
main(int argc, char* argv[])
{
    ...
    int fRunType, IsBar, FoundGen, s61, s74, s1, s75, s67;
    ...
    lambdaRegGenTree->Branch("fRunType", &fRunType, "fRunType/I");
    ...
    lambdaBarGenTree->Branch("fRunType", &fRunType, "fRunType/I");
    ...
    lambdaTotGenTree->Branch("fRunType", &fRunType, "fRunType/I");
    ...
    lambdaRegBGTree->Branch("fRunType", &fRunType, "fRunType/I");
    ...
    lambdaBarBGTree->Branch("fRunType", &fRunType, "fRunType/I");
    ...
    lambdaTotBGTree->Branch("fRunType", &fRunType, "fRunType/I");
    ...
    while (H1Tree::Instance()->Next() && !opts.IsMaxEvent(eventCounter)) {
            ...
            for (Int_t gtrloop = 0; gtrloop < gtrdata.GetEntries(); gtrloop++) {
                ...
                if ((part->GetPDG() == -211) && (part->GetMother2()<0) && (part->IsStr()==1)) {
                    if (gtrdata[part->GetMother1()]->GetPDG() == 3122) {
                        ...
                        //PionNucIntCorrLambdaRegIndex.push_back({ gtrloop, tempweight });
                ...
                if ((part->GetPDG() == 211) && (part->GetMother2() < 0) && (part->IsStr() == 1)) {
                    if (gtrdata[part->GetMother1()]->GetPDG() == -3122) {
                        ...
                        double tempweight = H1NuclIACor::GetWeight(1, 1.0, part->GetFourVector().Vect().Pt(), part->GetFourVector().Vect().Phi(), part->GetFourVector().Vect().Theta(),0.0, PVzgen);
                        //cout << "Weight: " << tempweight << endl;
                        //PionNucIntCorrLambdaBarIndex.push_back({ gtrloop, tempweight });
        ...
        ////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        ////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        // Now before I go into the main loop, if the data is MC, I am collecting the H1NuclIACor weights for all Lambda candidates
        // This correction is for pion tracks (Using pion daughters of Lambda candidates) only and reweights data to give a more accurate detector simulation by using a probability of reconstructing/not reconstructing a pion track with more realistic detector sim
        // A certain weight needs to be applied for accepted candidates (just the weight) and 1-weight for rejected candidates
        // Ex. For an event with Two candidates where we accept 1 and reject two, the event is taken with total weight = (w1)*(1-w2)
        // On Truth/generator level the total weight = (w1)*(w2) + (w1)*(w2-1) + (w1-1)*(w2) + (1-w1)*(1-w2) = 1, unchanged
        if (fRunType != 0) {
            for (Int_t NucIntloop = 0; NucIntloop < LambdaCand.GetEntries(); NucIntloop++) {
                H1PartMC* part = gtrdata[NucIntloop];
                //Uses Class H1NuclIACor, GetWeight method
                //takes truth level values as input  
                //static Float_t  GetWeight(int ipart, float charge, float pt, float phi, float theta, float dca, float z0)
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// Now before imposing any cuts, If this is MC data, I will check 
        if (fRunType != 0) {
