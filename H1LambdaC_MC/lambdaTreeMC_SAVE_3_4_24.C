...
main(int argc, char* argv[])
{
    ...
    static H1ShortPtr RunType("RunType"); //0=Data, else is MC
    ...
    double pT, Eta, x, invMasslambda, Q2s, Ys, Angle, v0chi2, weight1, weight2;
    ...
    while (H1Tree::Instance()->Next() && !opts.IsMaxEvent(eventCounter)) {
        ...
        //define vectors for storing gen level particle information
        vector<Int_t> GenLambdaIndices={};
        vector<double> GenvsRecAngleDiffs = {};
        ...
        //If this is MC data, loop over generated particles in the event and store all the lambdas
        
        if (*RunType != 0) {
            for (Int_t i = 0; i < gtrdata.GetEntries(); i++) {
                H1PartMC* part = gtrdata[i];
                if (part->IsGtr() == false) { continue; } //only look at V0 particles
                if (part->GetPDG() == 3122 || part->GetPDG() == -3122) {
                    GenLambdaIndices.push_back(i);
                }
            }
        }
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        //If this is MC data, compare gen and reconstructed momentums to find
        //gen lambda hypothesis for migration plots
        if (*RunType != 0 && GenLambdaIndices.size()!=0) {
            for (Int_t a = 0; a < GenLambdaIndices.size(); a++) {
                TVector3 LambdaPgen = gtrdata[GenLambdaIndices[a]]->GetFourVector().Vect();
                double genvsrecangle = (180 / (TMath::Pi())) * acos((LambdaPgen.Dot(LambdaP)) / ((LambdaPgen.Mag()) * (LambdaP.Mag())));
                //cout << "Gen vs Rec Angle: " << genvsrecangle << endl;
                GenvsRecAngleDiffs.push_back(genvsrecangle);
            }
            int genindex = minfinder(GenvsRecAngleDiffs);
            //cout << "GenIndexofMin: " << genindex << endl;
            //cout << GenvsRecAngleDiffs.size() << endl;
            
            //USE THIS TO CHECK ANGLE FOR MIGRATION PLOTS
            //cout << "Angle Diff: " << GenvsRecAngleDiffs[genindex] << endl;
            //cout << "Gen pT: " << gtrdata[GenLambdaIndices[genindex]]->GetPt() << endl;
            //cout << "Reconstructed pT: " << pT << endl;
