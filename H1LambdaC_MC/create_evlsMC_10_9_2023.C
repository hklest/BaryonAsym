...
main(int argc, char* argv[])
{
    ...
    double elecPhi, elecTheta, Epz, elecE, x, Q2e, Ye, RTheta, ClusterRadius, Zvertex; 
    ...
    //Commented this part out for the MC version
    ...
    while (H1Tree::Instance()->Next()) {
       ...
       /* scatelCount = 0;
        ...
        if (*q2e < 5 || *q2e > 100) { continue; } // 5 < Q2 [GeV] < 100
        Q2Events++;
        
        if (*ye < 0.1 || *ye > 0.6) { continue; } // 0.1 < y < 0.6
        yEvents++;
    ...
    TFile fileEventDist("/nfs/dust/h1/group/gtustin/h1oo/H1LambdaC_MC/EventDists06_MC.root","RECREATE");
    EventTree->Write();
    fileEventDist.Write();
    fileEventDist.Close();

    cout << "Eventlist written to " << opts.GetOutput() << endl;
    cout << "Event Dists written to " << "/nfs/dust/h1/group/gtustin/h1oo/H1LambdaC_MC/EventDists06_MC.root" << endl;
