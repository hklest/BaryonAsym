...
main(int argc, char* argv[])
{
    ...
    H1EventList *listLambdaCtoLambdaPion = new H1EventList("listLambdaCtoLambdaPion");
    H1EventList* listLambdaCtoK0shortProton = new H1EventList("listLambdaCtoK0shortProton");
    H1EventList* listLambdaCtoProtonKaonPion = new H1EventList("listLambdaCtoProtonKaonPion");
...
//        //Alignment fSpacAlignment;
 ...
 if (RunPeriod == H1Constants::eEplus9900)  cout << "e+ 99/00"
     ...
     cout << RunPeriod << endl;
          ...
          listLambdaCtoLambdaPion->Enter();
          listLambdaCtoK0shortProton->Enter();
          listLambdaCtoProtonKaonPion->Enter();
    ...
    TFile file1("LambdaCtoLambdaPionEVLS.root", "RECREATE");
    listLambdaCtoLambdaPion->Write();
    listLambdaCtoLambdaPion->Print();
    // close
    file1.Write();
    file1.Close();
    // Write event lists to files
    TFile file2("LambdaCtoK0shortProtonEVLS.root", "RECREATE");
    listLambdaCtoK0shortProton->Write();
    listLambdaCtoK0shortProton->Print();
    // close
    file2.Write();
    file2.Close();
    // Write event lists to files
    TFile file3("LambdaCtoProtonKaonPionEVLS.root", "RECREATE");
    listLambdaCtoProtonKaonPion->Write();
    listLambdaCtoProtonKaonPion->Print();
    EventTree->Write();
    // close
    file3.Write();
    file3.Close();
    ...
    cout << "Eventlist written to output " << endl;
    //cout << "Event Dists written to " << opts.GetOutput() << endl;
